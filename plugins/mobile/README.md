...
mobile は、iPhone/iPad アプリを設計・実装・レビュー・配布するときに使う2つのスキルを提供します。Apple Human Interface Guidelines（HIG）、Apple Developer Documentation、App Review Guidelines を一次資料とし、Codex が次の区別を保ったまま判断できるように構成します。
...
UIを伴うiOS/iPadOS実装では、原則として両方を使用します。`developing-ios-apps` が実装・品質・配布を担当し、`applying-apple-hig` がユーザー体験とインターフェイス判断を担当します。
