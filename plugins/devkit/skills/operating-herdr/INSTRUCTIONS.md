...
> **⚠️ `--split` の分割対象は「現在フォーカスされているpane」であり、CLIを呼び出したpaneでも「直前に作ったpane」でもない**: `agent start` には分割対象を明示するオプション（`pane split` の位置引数のような `target_pane_id`）が存在しない。そのため `--no-focus` のまま複数体を連続起動すると、フォーカスが動かない親pane（または最初に起動したpane）が毎回分割対象になり続け、新paneが親の周りに積み重なっていく（レイアウトが乱れる典型的な原因）。分割対象を切り替えたい場合は、次の agent start の直前に `herdr agent focus <直前のagent名>` で対象paneへ実フォーカスを移す。複数体を整列よく起動する手順は下記「複数エージェントを整列よく起動する」レシピを参照。
...
> **⚠️ Escape で中断した直後の多段送信は要注意**: 長時間動作中のエージェントを Escape で中断した直後に新しい指示テキストを `agent send` で送ると、通常のプロンプト送信として確定するとは限らない。対話型 UI が「プランを作成しますか」のような確認ダイアログ（モード切替キーや dismiss キーの選択肢）を表示したまま止まり、送ったテキストが入力欄に反映されないことがある。`agent send` → `pane send-keys Enter` を機械的に連投する前に、まず `herdr agent read <target> --source recent` や `herdr pane read <pane_id> --source recent` で現在の UI 状態を確認する。確認ダイアログが出ている場合は、そのダイアログを明示的に解決（選択肢に応じたキー入力を送る）してから本来送りたい指示を送ること。中断して全く別の独立タスクに切り替えたい場合は、同じ pane へ多段送信するより新しい `herdr agent start` で別 pane を起動する方が確実。
...
`agent start` は現在フォーカスされているpaneを分割するため、`--no-focus` のまま何体も連続起動すると全員が同じ親paneを分割し続け、新paneが親の周りに積み重なってレイアウトが乱れる。1体目は親の右に、2体目以降は「直前に起動したagentをfocusしてから下に分割」を繰り返すことで、親の右列に縦一列で整列させる。

```bash
# 1体目: 親（現在フォーカス中のpane）の右に分割
herdr agent start agent1 --cwd ~/project --split right --no-focus -- claude --permission-mode auto --name agent1

# 2体目以降: 直前に起動したagentをfocusしてから下に分割する
herdr agent focus agent1
herdr agent start agent2 --cwd ~/project --split down --no-focus -- claude --permission-mode auto --name agent2

herdr agent focus agent2
herdr agent start agent3 --cwd ~/project --split down --no-focus -- claude --permission-mode auto --name agent3

# 全員起動後、親にフォーカスを戻す（親 | 右列の2カラム構成なので left で一発戻れる）
herdr pane focus --direction left --current
```

`herdr agent focus <name>` はagent名から対象paneへ実フォーカスを移す（`pane focus` CLIには絶対pane_id指定の直接フォーカスがなく `--direction` の相対移動のみのため、agent名ベースの `agent focus` がこの用途で唯一の手段）。フォーカス切り替えは一瞬だけ画面上のハイライトを動かすが、最後に親へ戻すことでユーザーの見た目上は元通りになる。
...
- `wait agent-status --status done` の解決が遅れる
...
### 対処2: 一意な完了マーカー文字列を出力させて `wait output` で直接待つ

`agent_status` の判定に頼らず、子に一意な完了マーカー文字列（例 `TASK_DONE:<一意ID>`）を出力させ、親は次のコマンドで直接そのテキストを待つ:

```bash
herdr wait output <pane> --match "TASK_DONE:<一意ID>" --timeout <ms>
```
...
**agent start の分割対象**: `--split` は常に「現在フォーカスされているpane」を分割する（起動元paneでも直前に作ったpaneでもない）。複数体を整列よく連鎖配置したい場合は「複数エージェントを整列よく起動する」レシピの通り、次を起動する直前に `agent focus <直前のagent名>` で対象を切り替える。
