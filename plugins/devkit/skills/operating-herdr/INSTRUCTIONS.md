...
> **`agent send` と `pane run` の違い**: `agent send` はリテラルテキストを書き込むだけ（Enter を送らない）。コマンドを実行させたい・Enter を付けたい場合は該当 pane に対して `herdr pane run <pane_id> <command>` を使う。
...
```bash
herdr agent start reviewer --cwd ~/project --split right --no-focus -- claude
