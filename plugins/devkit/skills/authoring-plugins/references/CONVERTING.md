...
Phase D: 品質チェック（Claude Code本体）- 最終検証
    ↓
Phase E: リリース（Claude Code本体）- バージョン更新・コミット・タグ
```
...
> **🔴 PDF入力時の注意**: PDFファイルをReadツールで読み込んではならない（コンテキスト圧迫によるcompaction発生）。内容確認が必要な場合もMarkdown変換後に行うこと。Phase AのPlannerタチコマが `pdf-to-markdown` で変換する。

#### 0.2 作業ディレクトリ作成

入力ファイル名（または複数ファイルの場合は代表名）から `{skill-provisional-name}` を決定し、作業ディレクトリを作成する:
...
>
> Workflow 実行時も同様で、`resumeFromRunId` のキャッシュは**完了エージェントのみ**を再生し failed は全量再実行されるため、「1エージェント=1出力ファイル Write」型なら出力の実在（`ls`）で残タスクを機械特定し、**残タスクだけの新規小型ワークフロー**として書き下ろす方が確実で安価。

---

### Phase A: 計画策定（Planner タチコマ・Opus）

Claude Code本体は Planner タチコマを起動してファイル変換から構造設計まですべてを委譲する。**Planner の完了通知を受けてから Phase B に進む。**

#### A.1 Planner起動（Agent ツール呼び出し）
...
**スキャン画像PDFの事前判定（変換着手前に必須）:**

変換に着手する前に、先頭数ページの**テキストレイヤー有無**を判定する。PyMuPDF で `len(page.get_text().strip())`（text_chars）と `len(page.get_images())`（images 数）を確認し、text_chars ≈ 0 かつ images ≥ 1 が続くならスキャン画像PDFと判定する（ファイルサイズ ÷ ページ数が数百KB/page 級であることも傍証になる）。
...
for i, page in enumerate(doc[:5]):
    ...
    print(f"page {i}: text_chars={text_chars} images={images}")
```

画像PDFはテキスト抽出器を替えても（`pdftotext` → PyMuPDF）無意味。OCR の要否は費用対効果で判断し、他の良質なソースが同ドメインを網羅していれば、当該ソースを除外する判断も正当である。
...
書籍全体を1本のMarkdownへ変換する方式は、巨大EPUBでの偽成功（パイプが pandoc 本体の失敗を握り潰す既知エラー）や蒸留処理の直列化を招く。EPUB は ZIP アーカイブなので、展開して章単位で扱うと安全かつ並列化できる:

```bash
unzip -oq book.epub -d /tmp/epub

# 1) 目次だけ先に取得（本文を読まずに「新規 or 既存追記」「配置プラグイン」の判定材料が揃う）
#    EPUB2: toc.ncx / EPUB3: nav.xhtml
python3 - <<'PY'
import xml.etree.ElementTree as ET
ns = {'n': 'http://www.daisy.org/z3986/2005/ncx/'}
r = ET.parse('/tmp/epub/OEBPS/toc.ncx').getroot()
def walk(np, d):
    t = np.find('n:navLabel/n:text', ns)
    if t is not None and d <= 1: print('  ' * d + t.text)
    for c in np.findall('n:navPoint', ns): walk(c, d + 1)
for np in r.find('n:navMap', ns).findall('n:navPoint', ns): walk(np, 0)
PY

# 2) content.opf の spine（読書順ファイル列）で章ファイルを特定し、章単位で変換
pandoc -f html -t gfm-raw_html --wrap=none /tmp/epub/OEBPS/ch01.html -o /tmp/book/ch01.md
```

- 目次先読みにより、既存スキルとの重複チェック・グルーピング判定を**本文読込前**に完了できる
- 章単位変換により、章ごとの並列蒸留・行数バジェット個別設定・部分失敗からの機械的再開（出力ファイルの実在 `ls` で残章を特定）が可能になる

**サニタイズ用ブロックリストの機械抽出（変換ルール 4.1 の機械化）:**

`content.opf` の Dublin Core メタデータから書名・著者名・出版社名を抽出し、蒸留プロンプトの禁止例リストと生成物検証の grep パターンの**両方**に流し込む:

```bash
python3 -c "
import re
x = open('/tmp/epub/OEBPS/content.opf', encoding='utf-8').read()
for tag in ['title', 'creator', 'publisher']:
    for m in re.finditer(r'<dc:%s[^>]*>([^<]+)</dc:%s>' % (tag, tag), x):
        print(tag + ':', m.group(1))
"
# 生成物の検証は多角パターンで（書名 | 著者姓・名それぞれ | 出版社 | 『』 | 書籍示唆表現）
/usr/bin/grep -rnE "<書名>|<著者姓>|<著者名>|<出版社>|『|』|本書|この章|前章|筆者|著者|this book" <生成物ディレクトリ>
```

**画像ベースEPUBの検出と対応:**

pandocでEPUBをMarkdown変換した結果にテキストがほとんどなく `![` による画像参照が大半を占める場合は**画像ベースEPUB**と判定する。

**画像ベースEPUBはスキルに変換できない。** 以下の手順でユーザーに通知し、変換を中止する:

1. AskUserQuestion でユーザーに画像ベースEPUBであることを通知する:
   ```python
   AskUserQuestion(
       questions=[{
           "question": "この書籍は画像ベースのEPUBです。テキストがほとんど含まれていないため、スキルへの変換ができません。\n\n- 対象: {epub_filename}\n- 画像ベースEPUBとは: ページが画像として格納されており、テキスト抽出ができない形式です",
           "header": "画像ベースEPUB（変換不可）",
           "options": [
               {"label": "了解（変換を中止）", "description": "この書籍のスキル変換を中止します"},
               {"label": "テキスト部分のみで続行", "description": "画像を無視し、わずかなテキスト情報のみでスキル作成を試みる（品質は低い可能性あり）"}
           ],
           "multiSelect": False
       }]
   )
   ```
   - **「了解（変換を中止）」の場合**: 変換処理を終了する
   - **「テキスト部分のみで続行」の場合**: pandocで変換済みのMarkdown（テキスト部分のみ）をそのままソースとして使用し、品質が限定的である旨を記録してから通常フローに進む

**URL変換（curl + pandoc）:**

1. まず `curl` でHTMLをダウンロード
2. `pandoc` でMarkdown変換
...
PDF/EPUB含め変換結果は目視確認（ページ/章が欠落なく本文抽出されているか確認）。

検証基準:
| 元形式 | 最小期待文字数 | 確認ポイント |
|--------|-------------|---------|
| PDF | ページ数 × 500 | 下回っていたら内容確認 |
| EPUB | チャプター数 × 1000 | 下回っていたら内容確認（テキスト不足は画像ベースの可能性あり → 「画像ベースEPUBの検出と対応」参照） |
...
1メッセージ内で複数の Agent ツール呼び出しを並列実行:
...
**複数スキルグループがある場合**: グループごとに上記Implementerを `name` フィールドで `implementer-1`〜`implementer-N` と命名して並列起動する（v2.1.178 以降 `team_name` は廃止され無視されるため、個別名は `name` で付与する）。
...
タチコマはセッション終了時に自動解散する。能動的に閉じる場合のみ SendMessage を使用する（v2.1.178以降、TeamDelete は廃止）:

```json
// 必要に応じて各メンバーにシャットダウン要求を送信
// SendMessage は deferred tool のため、使用前に ToolSearch("SendMessage") でロードする
SendMessage({
  "type": "shutdown_request",
  "recipient": "planner",
  "content": "変換作業が完了しました。シャットダウンしてください。"
})
// 各 implementer にも同様に送信。shutdown_approved / teammate_terminated で確認する
```
...
Phase D 完了後、INSTRUCTIONS.md の「🔴 完了ワークフロー」に従い以下を実行する。
...
> `tag.gpgsign=true` の環境ではタグは常に annotated 扱いになりメッセージ必須。`-m` なしの軽量タグは非対話実行下で `fatal: no tag message?` になる。
...
| 除去対象 | 変換後の記述 |
|---------|------------|
| 「ある技術書の第3章によると...」 | 「関数設計のベストプラクティスとして...」 |
| `> -- ある開発者` | `> 関数は一つのことだけを行うべきである` |
| 「ISBN 978-xxx」 | （完全に除去） |

**素材ファイルの命名（混入経路を最初から断つ）:**

抽出・変換した素材ファイルの名前は、**書名を含まないジェネリック名**（`src-01.md` 等）にする。抽出物本文には著者名・出版社・ISBN・ページヘッダ・コード行番号が残ることが多いため、下流の蒸留プロンプトで除去を明示指示し、完成物は本体が grep で最終サニタイズする。
...
- `"Converts markdown files into well-structured Claude Code Skills. Use when creating new skills from existing markdown source material. Reference authoring-plugins for general skill creation guidelines."`
...
**既存スキル側の 1024 字予算が尽きている場合（description 手術）:**

既存スキルの description が上限近傍（実例: 995字・1,011字）だと、差別化文言の追記余地がない。以下の順で対処する:

1. **追記前に実測する** — `yaml.safe_load` でパースした実効文字列長で残余バジェットを確認する（ファイル上の折返しテキストを目視で数えない）
2. **違反句の除去で捻出する** — 既存 description 内の規約違反句（自スキル内部ファイル名への参照等）や冗長句を除去・圧縮して予算を作る（NAMING.md 違反の同時解消になる）
3. **本文へ降格する** — それでも収まらない場合は既存スキル側の frontmatter を触らず、**本文にルーティング行を追加**する。新規スキル側 description の片方向差別化＋既存スキル本文の相互参照で成立させる（双方向 frontmatter は理想だが、上限超過や既存 description の劣化を対価にしない）
