...
- hook（`detect-project-skills.sh` 等）も同様に **3 サイト**（GROUP 配列リテラル・`get_skill_description` の case ラベル・`PROJECT_SKILLS+=` push サイト）を**すべて同じ修飾文字列に揃える**。push が `lang:developing-react` なのに case ラベルが `"developing-react"` のままだと、エラーを出さず空 description になる（サイレント破損）。
