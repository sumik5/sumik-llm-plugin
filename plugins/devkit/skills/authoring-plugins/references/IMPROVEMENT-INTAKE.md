...
import yaml, pathlib
sk = pathlib.Path(f'plugins/devkit/skills/{skill}/SKILL.md').read_text()
...
```
tachikoma-doc-document-1: plugins/devkit/skills/skill-a/ 配下のみ
tachikoma-doc-document-2: plugins/devkit/skills/skill-b/ 配下のみ
...
/usr/bin/grep -rn "$SKILL_NAME" plugins/devkit/skills/*/SKILL.md

# Layer 2: 他スキル本文
/usr/bin/grep -rn "$SKILL_NAME" plugins/devkit/skills/*/INSTRUCTIONS.md plugins/devkit/skills/*/references/

# Layer 3: hook
/usr/bin/grep -n "$SKILL_NAME" plugins/devkit/hooks/*.sh

# Layer 4: README / rules / 自スキル内
/usr/bin/grep -rn "$SKILL_NAME" README.md rules/ "plugins/devkit/skills/$SKILL_NAME/"
...
python3 -c "import json; print(json.load(open('plugins/devkit/.claude-plugin/plugin.json'))['version'])"
...
> ⚠️ `plugins/devkit/.claude-plugin/plugin.json` を更新したら `.codex-plugin/plugin.json` も**必ず同じ値に同期**する（片方だけ更新すると配布物の整合性が崩れる）。
...
/usr/bin/grep -nE \
  ...
  plugins/devkit/skills/*/SKILL.md \
  plugins/devkit/skills/*/INSTRUCTIONS.md \
  plugins/devkit/skills/*/references/*.md \
