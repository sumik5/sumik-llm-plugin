...
## 🔴 Agent Skills 標準 vs Codex vs Claude Code 拡張

スキル定義は **Agent Skills 標準**（<https://agentskills.io/specification>）に準拠することが基本。
Codex は標準フィールドをそのまま使い、プラグイン配布では `.codex-plugin/plugin.json` と任意の `agents/openai.yaml` を追加で参照する。
...
| 項目 | Codex の扱い |
|------|--------------|
| 探索場所 | repo/user/admin/system の skill directory。repo では `$CWD/.agents/skills` から repo root までの `.agents/skills` を探索 |
| 初期一覧 | `name` / `description` / path を初期コンテキストへ入れる。上限はコンテキストの 2%（不明時 8,000 chars）で、長い description から短縮される |
| 暗黙呼び出し | `description` との一致で発火。抑止は skill 内の `agents/openai.yaml` に `policy.allow_implicit_invocation: false` を置く |
| UI/依存メタデータ | `agents/openai.yaml` の `interface` / `policy` / `dependencies` を使う |
| strict validation | 配布経路で挙動が分かれる。**git ベースの Codex plugin marketplace 経由（`codex plugin add`）は未知フロントマターキーを黙殺**（`context`/`agent`/`when_to_use`/`disable-model-invocation` 等の CC 拡張を含むプラグインも `installed, enabled` になりハードエラーなし・本 repo で実証済み）。一方 `skills-ref validate` / OpenAI API upload は**標準フィールド以外で失敗する** |
...
| `disallowed-tools` | 稼働中にツールをプールから除外（例: background loop で AskUserQuestion を封じる） | Claude Code のみ |
...
- **Codex 配布も対象**なら、重要なルーティング語は必ず `description` に入れる。Codex は `when_to_use` を標準フィールドとして扱わない
- `disable-model-invocation: true` は Codex の implicit invocation 抑止にならない。Codex では `agents/openai.yaml` の `policy.allow_implicit_invocation: false` を使う
- OpenAI API や `skills-ref validate` に通す必要がある skill は、Claude Code 拡張フィールドを frontmatter に直置きしない
- 🔴 **逆に、git ベースの Codex plugin marketplace のみで配布する skill（本 repo の配布経路）では、Codex 互換のために CC 拡張フィールドを剥がす必要はない**。Codex は未知キーを黙殺するため `context: fork`・`disable-model-invocation`・`when_to_use` 等は残したまま両クライアントで機能する（剥がすと Claude Code 側の必須機能が壊れる）。標準の `metadata:` に寄せるべきなのは、UI/ツールが読む**独自メタデータ**（バージョン・出典等）であり、CC 挙動フィールドではない
...
- ⚠️ **標準では `name` 必須**だが、**Claude Code では任意**（省略時は親ディレクトリ名にフォールバック）。クロスクライアント配布では明示記載を推奨

### description 三部構成（🔴 最重要）

descriptionはCodex/Claude Code本体がルーティング先を判断する**最重要の手がかり**。
...
- 🔴 **`description` は 1,024文字以内**（Agent Skills 標準制限・クロスクライアント互換のため変更不可）
- Claude Code では `description` と `when_to_use` の**合算**が skill listing で評価される。合算の既定上限は **1,536文字**（`skillListingMaxDescChars` で調整可能。標準の 1,024 description 上限そのものではない）
- **「標準の `description` 1,024 は維持。変動するのは Claude Code 側の合算 listing cap（既定1,536）のみ」**
- Codex では初期 skill 一覧の予算超過時に description が短縮される。先頭 1〜2 文だけで「何をするか」「いつ使うか」「主要な除外条件」が伝わるように書く
- Codex は `when_to_use` を見ない前提で書く。`when_to_use` にしかないトリガー語・差別化文は Codex では欠落する
- 具体的なタスク・技術名を列挙する（「helps with X」はNG）
- 🔴 差別化参照（For X, use Y instead）の Y は**実在スキルのみ**。クロスプラグイン参照は `plugin:skill` 修飾、同一プラグイン内は bare 名（詳細: [NAMING.md](references/NAMING.md) の「クロスプラグイン参照の修飾規則」）
- 同一ターゲットスキルへの差別化文は 1 文に統合する。自スキルの内部ファイル名（`references/X.md` 等）を description に書かない
- 日本語 description の折返しは文境界（。の直後）または既存の半角スペース位置のみ（folded scalar は折返しを半角スペースへ変換するため）。改修時は既存の主要言語（日本語/英語）を維持し勝手に翻訳しない
- 本文・references/ を増強・削減したら description の機能列挙も同期する（内容ドリフト防止。詳細: NAMING.md の「description の内容ドリフト監査」）

### 設定による上限制御（Claude Code 固有）

Claude Code 側の skill listing 挙動は以下の設定で制御できる（CLAUDE.md または プロジェクト設定）:

| 設定項目 | 説明 |
|---------|------|
| `skillListingMaxDescChars` | `description + when_to_use` 合算 listing cap の調整（既定1,536）。標準の description 1,024上限を変えるものではない（旧称 `maxSkillDescriptionChars` は現行ドキュメントで本名に改称） |
| `skillListingBudgetFraction` | スキル一覧掲載に割く文脈予算の割合（例 `0.02` = モデル文脈窓の2%）。環境変数 `SLASH_COMMAND_TOOL_CHAR_BUDGET` で固定文字数指定も可 |
| `skillOverrides` | スキル露出の制御: `on`（全表示）/ `name-only`（名前のみ）/ `user-invocable-only`（ユーザー呼出し可のみ）/ `off`（非表示） |

> 予算超過で description が短縮・欠落しているかは `/doctor` で診断できる（何スキルが短縮/ドロップされたかと対象を表示）。`/context` の Skills 行は予算適用後のサイズを報告する（v2.1.196+）。
...
- TypeScript/Python系のみ: `mastering-typescript` を追加
...
**Codex の暗黙呼び出しを禁止したい場合（標準 frontmatter は維持）:**
...
スキル・コマンドの本文内で以下の変数が動的に展開される:

| 変数 | 説明 | 注意事項 |
|------|------|---------|
| `${CLAUDE_SKILL_DIR}` | 当該スキルのバンドルディレクトリ（自スキルへのスクリプト参照の**公式推奨**・リネーム耐性あり） | ⚠️ 自スキルにのみ解決。他スキル参照は `${CLAUDE_PLUGIN_ROOT}/skills/<name>/` を使う。プラグインスキルではプラグインルートではなくスキルのサブディレクトリに解決 |
| `${CLAUDE_PLUGIN_ROOT}` | プラグインルートディレクトリ | 既出だが `${CLAUDE_SKILL_DIR}` との対比のため再掲 |
| `${CLAUDE_PROJECT_DIR}` | プロジェクトルートディレクトリ（hook・MCP が受け取る `CLAUDE_PROJECT_DIR` と同一パス） | 🆕 v2.1.196+。プロジェクトローカルスクリプト参照（例 `${CLAUDE_PROJECT_DIR}/.claude/hooks/helper.sh`）に。本文と `allowed-tools` 両方で展開されるため権限ルールとパスが一致する |
| `${CLAUDE_SESSION_ID}` | 現在のセッションID | セッション固有の一時ファイル名等に活用 |
| `${CLAUDE_EFFORT}` | 現在の effort レベル（`low` / `medium` / `high` / `xhigh` / `max`） | 処理量・詳細度の動的調整に活用 |
| `$ARGUMENTS[N]` または `$N` | 位置引数の N 番目（添字付き個別参照） | `$ARGUMENTS` で全引数を参照可。`$1`, `$2` 等で個別参照 |
...
- [ ] Codex strict / OpenAI API 配布が対象なら、frontmatter が標準フィールド（name / description / license / compatibility / metadata / allowed-tools）のみか
- [ ] Codex で手動呼び出し限定にしたい場合、`agents/openai.yaml` に `policy.allow_implicit_invocation: false` を設定したか
