...
| strict validation | `skills-ref validate` / OpenAI API upload では標準フィールド以外の frontmatter が失敗する |
...
- Claude Code では `description` と `when_to_use` の**合算**が skill listing で評価される。合算の既定上限は **1,536文字**（`maxSkillDescriptionChars` で調整可能。標準の 1,024 description 上限そのものではない）
...
| 設定項目 | 説明 |
|---------|------|
| `maxSkillDescriptionChars` | `description + when_to_use` 合算 listing cap の調整（既定1,536）。標準の description 1,024上限を変えるものではない |
| `skillListingBudgetFraction` | スキル一覧掲載に割く文脈予算の割合 |
...
| 変数 | 説明 | 注意事項 |
|------|------|---------|
| `${CLAUDE_SKILL_DIR}` | 当該スキルのバンドルディレクトリ（自スキルへのスクリプト参照の**公式推奨**・リネーム耐性あり） | ⚠️ 自スキルにのみ解決。他スキル参照は `${CLAUDE_PLUGIN_ROOT}/skills/<name>/` を使う |
