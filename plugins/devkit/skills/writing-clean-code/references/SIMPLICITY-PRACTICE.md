...
簡潔性の法則は、デザイン方程式の Em（管理にかかる作業量）を減らすための具体的手段である。

**Em削減のメカニズム**:
- コードを簡潔にすることで管理コスト（Em）が減少する
- Em が減ると、可能な変更に対する欲求度（D）が高まる（D ≈ Vf/Em より）
- 未来を予測せずとも、コードを見直して複雑な箇所を減らすだけで Em を削減できる
- この継続的な簡潔化こそが、管理コストを時間とともに減少させるメカニズムである
...
人は生まれつき簡潔なシステムを構築するようにはできていない。デザインに注意しないと、システムは複雑な獣に進化する。

**デザインの不可視性**:
- デザイン上の壊滅的な失敗は大規模で目につきやすい
- 良いデザインのための小さな努力の積み重ねは、コードに身近な人以外にはわかりにくい
- 大失敗の処理は感謝されるが、失敗の予防は誰にも気づかれない

**デザインの必要性**:

| デザインなし | デザインあり |
|---|---|
| 新機能追加ごとに複雑性が倍加 | 新機能追加前にリデザインで簡潔性を維持 |
| いつかプロジェクトが複雑性に圧倒される | 継続的な改善で管理しやすさが向上 |
| 「とりあえず動く」→ 将来の管理が困難 | インクリメンタルなデザイン → 長期的に効率的 |
