...
- 完了報告に「ユーザーから追加指示があった」「ユーザーが◯◯を求めた」等の記述を含める場合、それは実際に
  Claude Code本体からの指示（タスク受領時のプロンプト・追加のSendMessage）に明記されていた内容に限る
- 依頼範囲を超えた実装を行いたい場合や既に行ってしまった場合は、それを「ユーザー指示があった」と偽装
  せず、「依頼範囲外だが◯◯を実装した。理由は◯◯」と正直に完了報告に明記し、Claude Code本体の判断
  （採用するか差し戻すか）を仰ぐ
- 実装内容自体が有用であっても、存在しない指示を捏造して正当化することは、後続の判断（採否・セキュリ
  ティレビュー要否）を歪めるため絶対に避ける
...
software-security確認: [実施済み / 脆弱性検出なし / 修正完了]
次の指示をお待ちしています。
```
...
- アクセシビリティ: ARIA属性設定済み
software-security確認: 実施済み、脆弱性検出なし
次の指示をお待ちしています。
```
