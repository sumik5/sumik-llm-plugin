...
| EPUB（画像ベース＝スキャン本） | `scripts/recognize-image-to-markdown` でVLM-OCR |
| PDF（テキスト埋め込み） | `scripts/recognize-image-to-markdown`（pdftotextで直接）または `pdf-to-markdown` |
| PDF（画像ベース＝スキャン本） | `scripts/recognize-image-to-markdown` でVLM-OCR（**必須**・テキスト抽出は本文0問になる） |
| JSON | 構造化済みデータとして直接処理（変換不要） |

> 🔴 **スキャン本（画像ベースPDF/EPUB）はテキスト抽出では本文が取れない**。Kindle等の電子書籍は本文がページ画像で、`pdf-to-markdown`/`pandoc` は目次・奥付しか拾えない（実測: 529ページのスキャンPDFで抽出4KB・目次のみ・Q&A 0件）。この場合は `scripts/recognize-image-to-markdown` によるローカルVLM-OCRが必須。判定法・モデル選択・resume・既知の罠・トークン予算は [OCR-CONVERSION.md](references/OCR-CONVERSION.md) を必読。
...
- **画像ベース（スキャン本）のOCR変換**: LM Studio が起動し **vision対応モデルがロード可能**であること／`poppler`（`pdftotext`・`pdftoppm`）・`Pillow`・`lmstudio` Pythonパッケージが利用可能であること（詳細は [OCR-CONVERSION.md](references/OCR-CONVERSION.md)）
...
# 画像ベースEPUB（スキャン本）: VLM-OCR（要 LM Studio + vision モデル）
${CLAUDE_SKILL_DIR}/scripts/recognize-image-to-markdown <input.epub> -o /tmp/<descriptive-name>.md --model "<vision-model-key>"
```
...
```bash
# 推奨: テキスト/画像ベースを自動判定する統合コンバータ
#   テキストPDF → pdftotext で直接変換（LM Studio不要） / スキャンPDF → pdftoppm + VLM-OCR に自動フォールバック
${CLAUDE_SKILL_DIR}/scripts/recognize-image-to-markdown <input.pdf> -o /tmp/<descriptive-name>.md --model "<vision-model-key>"
#   スキャン本と分かっている場合は --pdf-force-ocr を付与して確実にOCR経路へ
# テキストPDFと確定している軽量ケースのみ pdf-to-markdown も可（LM Studio不要・高速）
${CLAUDE_PLUGIN_ROOT}/scripts/pdf-to-markdown <input.pdf> /tmp/<descriptive-name>.md
```

> 🔴 **画像ベースかの検証を怠らない**: `pdf-to-markdown`/`pandoc` の出力が「ページ数の割に極端に小さい（例: 529ページで 4KB・目次のみ・Q&A 0件）」なら本文は画像。`recognize-image-to-markdown --pdf-force-ocr` でOCRし直す。詳細・モデル選択・resume・既知の罠は [OCR-CONVERSION.md](references/OCR-CONVERSION.md)。
