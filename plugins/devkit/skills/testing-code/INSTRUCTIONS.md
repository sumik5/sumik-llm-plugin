...
- React Testing Library（RTL）関連 → **`developing-react`** スキルに移動済み
...
Vitest 固有の設定・移行・Browser Mode・カバレッジ等の詳細は testing-with-vitest スキル、RTL/React コンポーネントテストは lang:developing-react スキルを参照。
...
`aroundEach` / `aroundAll`（v4.1 追加）などの詳細なライフサイクルフック仕様・フックの実行順・globalSetup は testing-with-vitest スキルの VITEST-LIFECYCLE を参照。
...
Vitest 固有の詳細は **testing-with-vitest** スキル参照（設定・v4 移行・Browser Mode・カバレッジ・Fixtures・Lifecycle）。

- **developing-react** - React Testing Library（RTL）、React TDDパターン、Vitest+RTL統合
- **writing-clean-code** - SOLID原則・テスタブルな設計原則
- **mastering-typescript** - 型安全なテストコード
- **testing-e2e-with-playwright** - Playwright E2Eテスト
