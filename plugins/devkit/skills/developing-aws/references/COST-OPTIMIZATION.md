...
## 14. サービス別コスト最適化実践パターン

SBI生命保険のDWH構築プロジェクト等の実事例に基づく、サービス別コスト削減テクニック。

### 14.1 S3データ転送・保存コスト削減

| テクニック | 効果 | 適用条件 |
|-----------|------|---------|
| **gzip圧縮転送** | 転送量＋保存量を同時削減 | マネージドサービスがgz対応（Glue, Aurora等） |
| **複数gzのZIP一括転送** | 転送回数削減＋イベント処理簡素化 | Lambda(EventBridgeトリガー)で解凍 |
| **大ファイルは個別gz** | Lambda 15分制限回避 | 解凍に15分以上かかるファイル |

**ZIPイベント駆動パターン:**

```
オンプレミス → ZIP転送 → S3格納 → EventBridge → Lambda(解凍) → S3(gz個別格納)
```

- 1 ZIPファイル = 1イベントで後続処理が簡潔に
- 実績: S3年間コスト**約90%削減**

### 14.2 Glue→Lambda置換パターン

**⚠️ Glue DPU最小課金の罠**: 1処理がほぼリソースを使わなくても最小課金単位分の時間が課金される。

| 判断基準 | Glue推奨 | Lambda推奨 |
|---------|---------|-----------|
| **データ量** | ペタバイト級〜エクサバイト級 | 小〜中規模 |
| **処理時間** | 長時間（最大48時間） | 短時間（最大15分） |
| **処理内容** | 高度なETL（Spark活用） | 単純な変換・ロード |
| **ライブラリ** | ETL用ライブラリ標準搭載 | 自前でLayer設定 |
| **ZIP解凍** | ✅ 標準対応 | ⚠️ 15分制限に注意 |

**開発標準化のポイント:**
- 言語をJava/Pythonに限定
- テンプレートLambdaファンクションを用意（コピー＆選択で実装）
- パラメータファイル化（テーブル数等を外部管理、ファンクション変更不要）

- 実績: ETL処理コスト**約90%削減**（約40万円/月→約4万円/月）

### 14.3 Athenaクエリコスト削減

**課金モデル**: スキャンデータ量 × $5/TB（失敗クエリは課金対象外）

| 手法 | 効果 | 実装方法 |
|------|------|---------|
| **データ圧縮** | スキャン対象量を大幅削減 | S3保存データをGZIP圧縮 |
| **パーティション分割** | 不要データのスキャン回避 | WHERE句でパーティション（日付等）指定 |
| **列指向変換** | 必要カラムのみスキャン | CSV→Parquet変換（Glue等で実施） |

### 14.4 DynamoDB詳細コスト最適化

| テクニック | 効果 | 適用場面 |
|-----------|------|---------|
| **Standard-IAテーブルクラス** | 保存コスト約60%削減 | ログ、注文履歴、ゲーム実績等の低頻度アクセスデータ |
| **オンデマンド→プロビジョンド切替** | 処理コスト削減 | アクセスパターン安定後に切替 |
| **アイテムサイズ最適化** | RCU/WCU消費削減 | RCU=4KB単位、WCU=1KB単位（超過で追加ユニット消費） |
| **データ二重保存回避** | 保存＋処理コスト削減 | 複数サービス間でのデータ重複を排除 |

### 14.5 Redshiftコスト最適化

| テクニック | 効果 |
|-----------|------|
| **Provisioned RI（前払い）** | 最大70%以上削減（全額前払い・3年） |
| **格納データ精査** | 重複テーブル削除で容量削減（実績: 3800→960テーブル） |
| **Serverless活用** | 初期構築・利用量不明時はServerlessで開始 |

**Provisioned vs Serverless判断:**

| 項目 | Provisioned | Serverless |
|------|------------|-----------|
| **運用** | ユーザーがクラスタ管理 | AWS自動管理 |
| **チューニング** | 手動で最適化必要 | 自動 |
| **コスト** | RI活用で安価（熟練度必要） | RPU従量課金（手軽） |
| **推奨** | 長期・安定利用 | 初期導入・利用量変動 |

### 14.6 その他サービス別コスト削減

| サービス | テクニック | 効果 |
|---------|-----------|------|
| **Aurora** | 開発/テスト環境の停止スケジュール | 非稼働時間分のコスト削減 |
| **Kendra** | インデックス数最小化（用途別権限でインデックス増加防止、Cognito+ACL） | 固定費削減（インデックス単位課金） |
| **Step Functions** | 運用管理ツール(JP1等)→Step Functions置換 | 実績: 月額運用コスト**98%削減** |
| **QuickSight** | 未使用アカウント定期棚卸し、環境間重複検出 | ユーザー単位課金の無駄排除 |

**マイクロサービスのコスト設計:**
- ECS/EKSの代替としてLambda + API Gateway Composite APIパターン
- API Gateway 29秒制限: SQS非同期処理 or Step Functions同期処理で回避
- EventBridgeでイベント発行側・受取側をデカップリング
...
- AWSコスト最適化に関する一般的なエンタープライズ実践（DWH構築・サービス別コスト削減事例）は、公開されているAWS Well-Architected Framework Cost Optimization Pillarおよび業界カンファレンス資料を参照すること。
