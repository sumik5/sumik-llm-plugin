...
### Migrate for Compute Engine（M4CE）詳細手順

M4CEは**vSphere / AWS / Azure** からCompute Engineへの移行を支援するGCP公式の無料ツール。エージェントレスでデータをストリーミング転送するため、移行中のダウンタイムを最小化できる。

#### M4CEの主要特徴

| 機能 | 詳細 |
|------|------|
| **エージェントレス** | 移行元VMへのエージェントインストール不要 |
| **ストリーミング移行** | アクセス頻度の高いデータを優先転送し、数分でGCP上での稼働が可能 |
| **移行テスト（テストクローン）** | 本番稼働を止めずに移行後の動作を事前検証可能 |
| **ロールバック対応** | 移行中に問題が発生した場合、移行元への書き戻しが可能 |
| **一括移行** | 複数VMをCSVランブックで一括処理 |
| **Windowsアップグレード** | Windows Server 2008 R2 → 2012 自動アップグレード対応 |
| **コスト最適化** | 移行前のリソース使用状況分析からサイジング推奨を提供 |

#### 対応ソース環境

| ソース環境 | 対応バージョン |
|----------|--------------|
| **VMware vSphere** | 6.0〜7.x |
...
# 1-2. M4CE Manager VMのデプロイ（Google Cloud Marketplace から）
# Marketplace → "Migrate for Compute Engine" → デプロイ
# Manager VMはm4ce-manager-{version}として自動作成される

# 1-3. 移行用サービスアカウントの作成
gcloud iam service-accounts create m4ce-migration-sa \
  --display-name="M4CE Migration Service Account"

# 必要なロールを付与
gcloud projects add-iam-policy-binding PROJECT_ID \
  --member="serviceAccount:m4ce-migration-sa@PROJECT_ID.iam.gserviceaccount.com" \
...
**Step 2: 移行元環境の設定（vSphereの場合）**

```bash
# M4CE ManagerのUIにアクセス
# https://{MANAGER_VM_IP}/

# ソース登録手順（コンソールUI操作）:
# 1. Sources → Add source → VMware vCenter
# 2. vCenter Server のIPとクレデンシャルを入力
# 3. 移行対象VMの一覧が表示されることを確認
```

**Step 3: Cloud Extension（パイプ役）の設定**

```bash
# Cloud Extensionのサイズ選択基準:
# - Small: 同時移行VM数 最大20台（テスト・小規模向け）
# - Large: 同時移行VM数 最大50台（本番・大規模向け）

# Cloud ExtensionはM4CE ManagerのUIから作成:
# Infrastructure → Cloud Extensions → Add Cloud Extension
# → 対象プロジェクト・VPCサブネットを指定
```
...
```csv
# ランブック（CSVファイル）の形式例
sourceVmName,targetProject,targetZone,targetMachineType,targetSubnet,osLicenseType
web-server-01,my-project,asia-northeast1-a,n2-standard-4,subnet-frontend-tokyo,COMPUTE_ENGINE_LICENSE_TYPE_DEFAULT
app-server-01,my-project,asia-northeast1-a,n2-standard-8,subnet-backend-tokyo,COMPUTE_ENGINE_LICENSE_TYPE_DEFAULT
```

```bash
# ランブックをCloud Storageにアップロード
gsutil cp migration-runbook.csv gs://MIGRATION_BUCKET/runbooks/

# M4CE ManagerのUIで移行ジョブを作成・実行:
# Jobs → Create Job → ランブックのCSVを指定
# → テストクローン実行（本番稼働を止めずに検証）
# → 問題なければカットオーバー（本番切り替え）を実行
```
...
| チェック項目 | 確認内容 |
|------------|---------|
| **ネットワーク設計** | Dedicated Interconnect / HA VPN でGCPとオンプレミスを接続済み |
| **VPC設計** | 移行先のサブネット・ファイアウォールルールが設定済み |
| **IAM設計** | M4CE用サービスアカウントの権限が最小限で設定済み |
| **サイジング検討** | M4CEのリソース分析レポートを確認しマシンタイプを決定 |
| **テストクローン実施** | カットオーバー前に必ずテストクローンで動作検証 |
| **切り戻し計画** | 問題発生時の移行元への書き戻し手順を文書化 |

#### ウェーブ移行（段階的移行）戦略

大規模移行では一括ではなく**ウェーブ（波）に分けた段階的移行**が推奨される:

```
Wave 1: 非クリティカルシステム（開発・テスト環境）
  → 移行ツール・手順の検証
  → チームのGCPオペレーション習熟

Wave 2: 低リスクの本番システム（内部ツール・管理系）
  → 移行効率の最適化
  → 監視・運用設計の実地検証

Wave 3: 業務クリティカルシステム（本番EC等）
  → 十分な移行知識を持った状態で実施
  → 切り戻し計画を最も厳密に準備

Wave N: レガシーシステム（改修が必要なもの）
  → アプリケーションの改修・コンテナ化と並行
```
...
| 方式 | 適合ケース | M4CEとの違い |
|------|----------|-------------|
| **M4CE（Lift & Shift）** | オンプレVM → GCP VM、最小変更 | 最も迅速・低リスク |
| **OVA/OVFインポート** | VMwareの仮想アプライアンスをそのまま移行 | 少量のVM向け |
| **カスタムイメージ作成** | OSのみ移行しアプリは再デプロイ | 高度なカスタマイズが必要な場合 |
| **コンテナ化（Migrate to Containers）** | VMのワークロードをGKEに移行 | モダナイゼーション（追加工数が必要） |
| **Database Migration Service** | DBのみの移行（MySQL/PostgreSQL → Cloud SQL） | データのみの移行（VMは含まない） |
