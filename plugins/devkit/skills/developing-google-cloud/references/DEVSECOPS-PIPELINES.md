...
Cloud Deployはデプロイに特化したフルマネージドCDサービス。オープンソースの **Skaffold** を利用して定義のレンダリングとパイプラインを分離し、柔軟かつ簡潔な定義を実現する。Cloud Build・Cloud Storage・Operations Suite・Pub/Sub等の実績あるサービスを内部活用しており、アーティファクトとバージョン管理に特化した設計。

**デリバリーのプロセス：**

1. Skaffoldで何（コンテナイメージ・Kubernetesマニフェスト等）をどのようにデプロイするか定義
2. **デリバリーパイプライン**（Cloud Deploy独自のYAML定義）でターゲットへのデプロイ順を定義
3. **ターゲット**（デプロイ先）を定義。各ターゲットで承認要否を設定可能
4. 上記をCloud Deployへ登録
5. CIパイプライン等からgcloudコマンドまたはAPI経由でCloud Deployを呼び出し、リリースと最初のターゲットへのロールアウトを作成
6. 次のターゲットへの準備が整ったら再度呼び出し、定義した順番で次のロールアウトを作成

**リリースとロールアウトの概念：**
- **リリース** — デプロイするリソースと各ターゲットへのデプロイ方法を含む一連のライフサイクル
- **ロールアウト** — リリース内の個々のターゲットに対するデプロイ。1リリースにdev/QA/productionなど複数ロールアウトが含まれる

**ロールバック：** ターゲットごとに過去のリリースへロールバック可能。過去に成功したデプロイのリソースとパラメーターを再デプロイする安定した仕組み。

**アクセス制御：** IAMでデリバリーパイプライン・ターゲット単位の権限管理が可能。IAM Conditionsを利用することで特定のターゲットのみへのロールアウトや承認を許可するような詳細な権限制御も実現。

**Pub/Sub通知：** リソース・オペレーション・承認の3つの情報をそれぞれ異なるPub/SubトピックへPublishして連携。

---

## Artifact Registry 機能拡張

### Container Registry との差分

Artifact RegistryはContainer Registryの後継サービス。Container RegistryではCloud Storageに対して権限管理や暗号鍵の設定が必要だったが、**Artifact Registryはストレージが隠蔽されておりArtifact Registryのみで設定が完結**する。

**主な機能拡張点：**

| 機能 | Container Registry | Artifact Registry |
|------|------------------|--------------------|
| サポートフォーマット | DockerイメージマニフェストV2・OCIイメージ | Docker・Helm・Maven・Gradle・npm・PyPI・Apt・RPM等 |
| ストレージロケーション | マルチリージョナルのみ | リージョナル/マルチリージョナルを選択可能 |
| リポジトリ分離 | プロジェクト単位 | 1プロジェクト内で複数リポジトリを権限分離して管理 |
| 権限管理 | Cloud StorageバケットのIAM | リポジトリ単位のIAM（最小権限で管理しやすい） |
| 暗号化 | Cloud Storage側で設定 | リポジトリ単位で設定可能 |
| ホスト名 | `gcr.io` | `pkg.dev` |

Container RegistryからArtifact Registryへ移行する際は、CI/CDなどのホスト名参照先の変更（`gcr.io` → `pkg.dev`）に注意。また、Artifact Registryのストレージコストは Container Registryより高くなる場合があるため、コスト試算を事前に行うこと。
