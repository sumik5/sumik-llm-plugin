---
name: securing-ai-development
description: >-
  Organizational security strategy for AI-powered software development covering
  trust frameworks, adaptive guardrails, AI-BOM, AI-SPM, governance models,
  and cross-functional ownership. Use when establishing security controls for
  AI coding assistants, agentic systems, or AI-accelerated SDLC workflows.
  For code-level security (OWASP, CodeGuard), use securing-code instead.
  For AI development methodology (prompts, context engineering), use
  developing-with-ai instead. For LLM-specific security (prompt injection, LLMSecOps), use ai:designing-genai-patterns.
---

詳細な手順・ガイドラインは `INSTRUCTIONS.md` を参照してください。
