...
Codex本体がAgentをWave単位で最大並列起動する2フェーズ方式のワークフロー。**Phase 1（計画策定・Wave分割）はtachikoma-product-manager agentに委譲し、Phase 2（実装）でWave内のagentを自然言語で全て同時起動する。** 実際の並列実行はCodexのネイティブ `max_threads`（デフォルト6）が制御する。

---

## Phase 1: 計画策定

### Step 1: tachikoma-product-manager agent 起動（即座に実行）

**🔴 Codex本体はファイルを読まない・コードベースを分析しない。**

ユーザー要求を受け取ったら、即座に tachikoma-product-manager agent を自然言語で起動する:

```
"Spawn tachikoma-product-manager agent with the following task:"
...
**tachikoma-product-manager agent の責務（全委譲）:**
...
**tachikoma-product-manager agent は実装コードを変更しない（読み取り専用 + docs/ への計画書作成のみ）。**

---

### Step 2: 計画レビュー・承認

tachikoma-product-manager agent が `docs/plan-{feature-name}.md` を作成しユーザーに提示する。
...
**ユーザーの応答に基づく対応:**
- **承認** → Phase 2 へ進む
- **修正要求** → tachikoma-product-manager agent を再起動し、ユーザーのフィードバックを渡して計画を修正
...
- [2026-02-18 10:00] tachikoma-product-manager agent 起動（計画策定）
...
```
tachikoma-product-manager（計画策定・Wave分割）
  → ユーザー確認
...
```
tachikoma-product-manager（調査計画策定）
  → ユーザー確認
...
```
tachikoma-product-manager（分析・移行計画策定・Wave分割）
  → ユーザー確認
...
```
tachikoma-product-manager（計画策定）
  → ユーザー確認
