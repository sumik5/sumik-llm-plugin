...
## STRIDE脅威モデルとサーバーレス固有のマッピング

STRIDEは脅威を6カテゴリに分類するフレームワークだが、サーバーレス環境では各カテゴリが従来と異なる形で現れる。

| STRIDE カテゴリ | サーバーレス固有の脅威 | 対応する脅威番号 |
|---------------|----------------------|---------------|
| **S**poofing（なりすまし） | Cognitoの自己登録悪用、IDトークン偽造、Identity Pool の過剰権限ロール悪用 | #12 Broken Authentication |
| **T**ampering（改ざん） | CI/CDパイプライン経由でのデプロイコード改ざん、過剰権限IAMロールによるLambdaコード置換 | #11 CI/CD, #6 Over-Privileged |
| **R**epudiation（否認） | ログ機構の無効化・KMSキー即時削除による証跡消去、CloudTrailの無効化 | #9 Insufficient Logging |
| **I**nformation Disclosure（情報漏洩） | SSTIによる環境変数の窃取、公開バケットからの機密ファイルダウンロード、フロントエンドコードへの認証情報埋め込み | #3 Leaked Credentials, #2 Storage |
| **D**enial of Service（サービス妨害） | 大量ファイルアップロードによるイベント駆動関数の過多呼び出し、コスト爆発攻撃（DoW） | #14 DoS/DoW |
| **E**levation of Privilege（権限昇格） | インジェクション→クレデンシャル窃取→IAMユーザー新規作成→アカウント掌握 | #5 Injection, #6 Over-Privileged |
...
サーバーレス関数はHTTPリクエスト以外にも多様なイベントソースからトリガーされる。攻撃者はAPIゲートウェイ以外の経路からもインジェクション攻撃を試みる。

| イベントソース | 攻撃ベクトルの例 |
|-------------|---------------|
| Cloud Storage / S3 | ファイル名にインジェクションペイロードを埋め込み、ファイルアップロードイベントで実行 |
| メッセージキュー（SQS、Pub/Sub） | 悪意あるペイロードをキューに投入し、コンシューマ関数を攻撃 |
| Webhook / 外部API | 信頼できるように見える外部データに実行可能なペイロードを混入 |
| スケジューラ（EventBridge） | 定期実行の関数を標的にした持続的な攻撃 |
| データストリーム（Kinesis、BigQuery） | ストリーム経由で悪意あるレコードを流入させる |

**すべてのイベントソースからの入力を信頼せず、検証・サニタイズすること。**

### 一時的実行環境のリスク

サーバーレス関数は短命なコンテナ上で動作するため、従来の永続化手法（ファイルシステムへのバックドア書き込み等）は無効になる。一方で以下のリスクがある。

- **実行環境の共有**: 同一アカウント内の複数関数が同じ基盤インフラを共有し得る（テナント分離の制限）
- **クレデンシャルの一時性**: 取得した一時クレデンシャルの有効期限内に攻撃を完遂する必要がある
- **IAMベースの永続化**: 環境変数やファイルシステムへの永続化は困難なため、攻撃者はIAMユーザー新規作成やアクセスキー追加発行による永続化に切り替える

**防御**: CloudTrailでIAMユーザー作成・アクセスキー発行イベントを監視し、異常時即時アラートを設定する。

### 共有責任モデルと設定ミスのリスク

クラウドプロバイダが提供するマネージドサービスは、デフォルト設定が必ずしもセキュアでない。

| リスク | 具体例 |
|------|------|
| デフォルト設定の危険性 | Cognito User PoolのAdminCreateUserOnly=false（自己登録デフォルト有効） |
| 開発便宜上の広権限 | 開発時に付与したadministrator相当のIAMロールが本番環境に残存 |
| 新サービスのセキュリティ機能遅延 | 新規リリースされたサービスには数ヶ月間セキュリティ機能が不足することがある |
| ステージング→本番リソース参照 | 設定ミスにより非本番環境から本番リソースへ誤ってアクセスできる状態 |

---

## 攻撃チェーンの詳細シナリオ

### シナリオ3: コードインジェクション起点の全アカウント侵害

攻撃者は一見無害に見える複数の脆弱性を連鎖させ、クラウドアカウント全体を掌握する。

1. **偵察**: Webフロントエンドのソースコードを検査し、API関数URLとCognito識別子を抽出
2. **認証バイパス**: Cognito自己登録機能を悪用して攻撃者アカウントを作成・認証
3. **コードインジェクション**: SSTIまたはeval()の脆弱性を利用し、OSコマンド実行経由で環境変数を窃取
4. **クレデンシャル窃取**: 環境変数に含まれるIAMロールの一時クレデンシャルを取得
5. **横移動**: 取得したクレデンシャルを使いS3/GCS/Storageへアクセス、設定ファイルから追加シークレットを発見
6. **権限昇格**: 過剰なIAMロールの権限を利用し、新規IAMユーザーをadministratorポリシーで作成
7. **永続化**: 新規IAMユーザーの長期アクセスキーを発行し、バックドアを確立
8. **検知回避**: CloudTrailのログ配信を無効化、または暗号化キーを即時削除してログを永久消去

**この連鎖を断ち切る防御ポイント:**
- ステップ1: フロントエンドに内部識別子を埋め込まない
- ステップ2: Cognito自己登録を業務要件がない限り無効化
- ステップ3: eval()/exec()の使用禁止、テンプレートエンジンのサンドボックス化
- ステップ5: S3 Block Public Access + IAM最小権限
- ステップ6-7: IAMアクセス解析機能（IAM Access Analyzer）とGuardDutyによる異常検知
- ステップ8: CloudTrailの組織レベル有効化とS3ログバケットへのオブジェクトロック設定

### シナリオ4: サプライチェーン→CI/CDパイプライン経由の本番侵害

1. **依存関係汚染**: 攻撃者が既存のOSSパッケージを乗っ取り、環境変数を外部送信するコードを混入
2. **パイプライン実行**: 開発チームが依存関係インストールを実行するとパイプライン環境のクレデンシャルが窃取される
3. **本番デプロイ**: 盗んだCI/CDクレデンシャルで本番Lambdaのコードを悪意あるバージョンに置換
4. **データ継続窃取**: デプロイされた悪意あるコードが処理するすべてのリクエストからデータを横取り
5. **長期潜伏**: ログ欠如・アラート不備のため数週間〜数ヶ月発覚しない

**防御**: SBOMによる依存関係管理、パッケージlock fileの固定、CI/CD環境のシークレット最小権限化、デプロイ前の署名検証。
