...
> RAGの理論的詳細（インデックス設計・類似検索アルゴリズム・評価手法）は `designing-genai-patterns` スキルを参照。
...
**注意**: インデックス作成時と検索時に**同じ埋め込みモデル**を使用すること。異なるモデルを使うと次元ミスマッチエラーが発生する（`designing-genai-patterns` スキルのトラブルシューティング参照）。
...
> より高度なRAGの実装パターン（データローダー・評価・本番運用）は `designing-genai-patterns` スキルを参照。
