...
ファイル名にプロバイダー名を含めることで、複数のLLMプロバイダー向けプロンプトを同一プロジェクト内で管理しやすくなる:

| 命名パターン | 例 | 用途 |
|------------|-----|------|
| `<タスク>_<プロバイダー>_v<バージョン>.ts` | `translation_openai_v1.0.1.ts` | OpenAI向けプロンプト |
| `<タスク>_<プロバイダー>_v<バージョン>.ts` | `translation_anthropic_v1.0.0.ts` | Anthropic向けプロンプト |

この命名規約により、同じタスクに対してプロバイダーごとの最適化が可能になり、モデル移行時のコード変更範囲を最小化できる。プロバイダーごとに語調・指示スタイルを変えたい場合にも対応しやすい。
