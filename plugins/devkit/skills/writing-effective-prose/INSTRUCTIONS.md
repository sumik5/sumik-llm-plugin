...
10. **書く心構え・キャリア戦略** -- 書く生活/書かない生活、AI時代の書き手の価値、複数収益源
11. **人を動かすビジネス文書（説得・行動喚起メソッド）** -- エッセイ本質、稟議書/始末書/学生文章、一人弁証法
12. **五感で書く・感性のトレーニング** -- 五感言語化、AIに書けない文章、名文の引き出し
13. **ノー推敲嫌われ文章パターン・推敲困ったら** -- 5パターン検出、推敲First Aid
14. **UXコピーライティング** -- マイクロコピー、機能vs情緒、キャッチフレーズ
...
文書の目的が明確でない場合、AskUserQuestionツールでユーザーに確認する。（AskUserQuestion が使えない環境（Codex CLI 等）では、同じ質問テンプレートの選択肢を通常のテキストで提示して回答を待つ）
...
書き続けるための心構えとAI時代のキャリア設計を体系化したガイド。

**参照ファイル**: [WRITING-MINDSET-CAREER.md](references/WRITING-MINDSET-CAREER.md)

カバー範囲: 書く生活と書かない生活の差、AI時代に問われる書き手の価値、書く仕事の種別マップ、3つの壁と突破口、個人ブランディング、コミュニティ活用、複数収益源、需要と供給の交わる場所、習慣化チェックリスト

---

### 人を動かすビジネス文書（説得・行動喚起メソッド）

エッセイ・学生文章・稟議書・始末書・一人弁証法など、「人を動かす」ための文章の技法。

**参照ファイル**: [PERSUASIVE-WRITING.md](references/PERSUASIVE-WRITING.md)

カバー範囲: 文章は「人を動かす」ための道具（核思想）、エッセイの本質（テーマ→捉え直し→発見）、学生のための文章術（読書感想文/小論文/自己アピール文）、メールの身体性とプラスα、一人弁証法、視点移動能力、引用の「おトク感」

---

### UXコピーライティング

UIコピー・マイクロコピー・キャッチフレーズ・ネーミングに特化した書き方ガイド。

**参照ファイル**: [UX-COPYWRITING.md](references/UX-COPYWRITING.md)

カバー範囲: マイクロコピー（ボタン・ラベル・エラーメッセージ・空状態）、機能的ライティング vs 情緒的ライティング、キャッチフレーズの作り方、ネーミング技術、依頼者を喜ばせる文章、「文章は不便なツール」前提の設計

---

### 五感で書く・感性のトレーニング

感性は鍛えられる——五感を使った表現技法とAIに書けない文章の作り方。

**参照ファイル**: [SENSORY-CRAFT.md](references/SENSORY-CRAFT.md)

カバー範囲: 感性の後天的訓練、五感（視覚・聴覚・触覚・嗅覚・味覚）で書く技法、「もの」に変換する（抽象→具体）、視覚に寄りかからない、安易な形容を排除する、名文の引き出しを持つ、AIに書けない文章の本質

---

### ノー推敲嫌われ文章パターン・推敲困ったら

嫌われる文章の5パターンを検出・修正し、推敲に詰まったときの応急処置を提供するガイド。

**参照ファイル**: [NO-REVISION-PATTERNS.md](references/NO-REVISION-PATTERNS.md)

カバー範囲: ノー推敲文章5パターン（接続詞過多・主述ねじれ・無駄な長文・専門用語・ウラなし）、推敲不要に近づける3テク（クセ把握・正しい言葉・構成先行）、推敲First Aid（敵対的読み・身近な読者・プロとの差分分析）
...
| [EXPLANATION-TECHNIQUES.md](references/EXPLANATION-TECHNIQUES.md) | 説明・伝達の技法（説明の基本モデル、論理的伝え方、相手意識を踏まえた伝達技法、図解技法、技術の伝え方） |
...
文章を読んで「なんか変」と感じた
        ...
        |
        +-- 「書き続けられない・どんなキャリアを描けばいいかわからない」
        |       -> 心構え・キャリア -> WRITING-MINDSET-CAREER.md
        |
        +-- 「人を動かす説得力ある文書を書きたい・エッセイ/稟議書/学生文章」
        |       -> 説得・行動喚起メソッド -> PERSUASIVE-WRITING.md
        |
        +-- 「ボタン文言・UIコピー・キャッチフレーズ・ネーミング」
        |       -> UXコピーライティング -> UX-COPYWRITING.md
        |
        +-- 「描写が平板・感性的に書けない・五感を使った表現」
        |       -> 五感で書く -> SENSORY-CRAFT.md
        |
        +-- 「自分の文章はなぜ読みづらいのか・推敲のコツがわからない」
                -> ノー推敲パターン -> NO-REVISION-PATTERNS.md
```
