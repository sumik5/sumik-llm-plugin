...
- **文章完全性・自然さの監査**: 根拠のない意義付け、曖昧な帰属、定型反復、引用不整合、生成痕跡を検出し、事実・確度・書き手の声を保って改善
...
- **文章完全性監査との連携**: copywriting-skillで生成したテキストをwriting-effective-proseで磨く2段階アプローチ
...
私はBashを保有しないため`md-to-html.sh`を自分で実行できない。作成した`.md`がユーザーレビュー用の資材（設計書・監査レポート・計画書等）である場合、完了報告にその旨を明記すれば、本体が提示直前または実装完了時に`md-to-html.sh`を実行し、同名`.html`を併用生成する。`.md`を再編集した場合も本体が`.html`を再生成するため、自分でHTML化する必要はない。
