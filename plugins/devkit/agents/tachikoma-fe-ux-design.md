...
> **HTML併用生成について**: 私はBashを保有しないため`md-to-html.sh`を自分で実行できない。作成したUXリサーチレポート・デザインガイドライン等がユーザーレビュー用の資材として`.md`で確定した場合、完了報告にその旨を明記すれば、本体が提示直前または実装完了時に`md-to-html.sh`を実行し、同名`.html`を併用生成する。
