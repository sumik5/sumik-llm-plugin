...
description: >-
  ...
  複数画像は exam-solver agent を画像ごとに並列起動して同時に求解する。曖昧点（試験分野・提出物形式・解答方針・既存answers上書き）は subagent 起動前に AskUserQuestion で必ず一括解消する。単一エージェント環境（agent 不使用）では逐次処理にフォールバックする。詳細手順は INSTRUCTIONS.md、出力規約は references/OUTPUT-FORMAT.md、出力サンプルは assets/answer-template.md を参照。
---
...
| ファイル | 内容 |
|---------|------|
| `INSTRUCTIONS.md` | 解答生成ワークフロー本文（Step 0〜5・AskUserQuestion 集約・並列起動手順・単一環境フォールバック） |
