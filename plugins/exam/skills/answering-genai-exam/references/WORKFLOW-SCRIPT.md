...
- git 書込・ブランチ操作は行わない。
- 実バイナリ（PNG/PDF/SVG）は生成しない（必要なら本体が Step 4.5 で後段レンダリングする）。
```
...
【禁止】特定試験ブランド名・主催団体名・書籍名・著者名・出版社名を一切含めない。git書込・ブランチ操作をしない。実バイナリ（PNG/PDF/SVG）は生成しない（本体が後段でレンダリングする）。`
...
> **実バイナリレンダリング（opt-in）**: Step 2 で実レンダリングが選ばれた場合、worker は指示文/原稿md のみ出力し、**本体が Step 4.5 として後段でレンダリング**する（Workflow 標準 agent はツールチェーン不定のため worker 内で実行しない）。手順は同階層の `RENDERING.md`。
