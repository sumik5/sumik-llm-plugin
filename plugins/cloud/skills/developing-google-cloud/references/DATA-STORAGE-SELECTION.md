...
## ゲームDBの選択パターン（スパイク耐性・レイテンシ要件別）

ゲームインフラはトランザクション量が多く、アクセススパイクが激しいため、一般的なWebサービスとは異なるDB選択の判断軸が必要になる。

### ゲームインフラの特性と要件

| ゲームの特性 | DB選択への影響 |
|------------|--------------|
| **アクセススパイク** | リリース直後・イベント時に通常の数十〜数百倍のトランザクションが発生。スケールアウト対応が必須 |
| **需要予測困難** | ヒット判定が事前にできないため、事前サイジング不可。従量課金・自動スケールが前提 |
| **レイテンシ要件** | ガチャ・バトル等はミリ秒レベルの応答が必要。読み書きのバランスによってキャッシュ戦略が変わる |
| **スキーマ変化** | ゲーム改良で追加機能が増え、DBスキーマが頻繁に変化する |
| **ライフサイクル** | 多くのタイトルは5年以内にサービス終了。コストと管理コストの最小化が重要 |

### ゲームタイプ別DBスタック選択

| ゲームタイプ | 推奨DBスタック | 選定理由 |
|------------|--------------|---------|
| **モバイルゲーム（ソーシャル系）** | Cloud SQL（MySQL互換）+ Firestore（セッション・キャッシュ） | RDB互換でゲームデータを管理しつつ、スパイク時のセッション管理をNoSQLで対応 |
| **リアルタイム対戦ゲーム** | Cloud Spanner（グローバル一貫性）+ Memorystore（Redis）キャッシュ | グローバルユーザーへの低レイテンシ + ランキング・マッチングのリアルタイム処理 |
| **パズル・軽量ゲーム（GAE前提）** | Firestore（Datastoreモード）+ Cloud Storage（アセット） | GAEとのネイティブ統合、スキーマレス、低レイテンシ |
| **大規模MMORPGなど** | Cloud SQL（主要データ）+ Bigtable（行動ログ・時系列）+ BigQuery（分析） | 用途別の最適なサービスを組み合わせ |
...
スパイク予測が困難で大きい（モバイルゲーム等）
  → Firestore / Cloud Spanner（自動スケール）
  → 自前MySQLは避ける（手動スケールアウトが必要）

スパイクは小さく予測可能（アーケードゲーム等）
  → Cloud SQL（管理しやすいRDB）
  → 適宜Read Replicaで読み取り分散

超大量トランザクション（秒間10万件以上のログ・行動記録）
  → Bigtable（Wide Column、大量行・少数列に最適）
  → BigQueryへの日次バッチ移送で分析基盤と連携
```

### レイテンシ要件別の選択基準

| レイテンシ要件 | 適切なサービス | ユースケース例 |
|-------------|--------------|--------------|
| **1ms以下** | GCE上のRedis（Memorystore）| ランキング更新、セッション管理、ガチャの結果キャッシュ |
| **数ms〜10ms** | Firestore（Datastoreモード） | ユーザープロファイル、インベントリ、フレンドリスト |
| **数十ms** | Cloud SQL（同一リージョン）| トランザクション整合性が必要なゲームデータ（課金・購入） |
| **グローバル均一低レイテンシ** | Cloud Spanner | グローバル展開のリアルタイム対戦ゲーム |
| **秒単位（分析用途）** | BigQuery | KPI算出、ユーザー行動分析、不正検知 |

### キャッシュ戦略（GCE上のRedis活用）

ゲームインフラではDB直アクセスではなく、Redisキャッシュを挟む構成が一般的。GCEではStartup Scriptでセットアップし、IP Forward機能でフェイルオーバーを実現できる。

```bash
# Startup Scriptを使ったRedisセットアップ
...
**Redisフェイルオーバーの仕組み（IP Forward使用）**:
- 仮想IPアドレス（例: 10.1.1.1）に対してIP Forwardを設定
- Redis障害時にルーティングの優先度を変更（gcloudコマンドでプライオリティ切り替え）
- Sentinel + IP Forwardの組み合わせで商用環境の高可用性を実現

### ディスク選択と性能要件（GCE上のDB用）

GCE上に自前でDBを構築する場合は、ディスク種別の選択が性能ボトルネックになる。

| ディスク種別 | IOPS性能 | ユースケース | 注意点 |
|------------|---------|------------|--------|
| **Persistent Disk（標準）** | 中 | ログ保存、標準DBデータ領域 | IOPSはディスクサイズに比例して増加 |
| **Solid-State Persistent Disk** | 高 | MySQL/RDB・DataStore系の保存先 | 20万QPSに満たない要件ならこれで十分 |
| **Local SSD** | 極高 | Massive DB要件（20万QPS超） | インスタンス停止でデータ消失。RAID構成推奨 |

**ストレージ階層の活用例（grasys実績）**:
```
直近1日のバックアップ → GCEアタッチディスク（PD or SSD）
直近3日のバックアップ → Cloud Storage（Standard）
それ以前のアーカイブ → Cloud Storage（Nearline） ← コスト半額以下
```

### GCSのゲームアセット配信活用

静的コンテンツの配信にはCloud StorageのエッジキャッシュがCDN代替として機能する。

| 用途 | 推奨構成 | 備考 |
|------|---------|------|
| **ゲームアセット（画像・音声）配信** | GCS + エッジキャッシュ + Cloud CDN | HTTP(S) LBと組み合わせてCDNを有効化 |
| **過去タイトルのリソースアーカイブ** | Cloud Storage Nearline | 数秒でデータ取り出し可能、コールドストレージとしてはGlacierより高速 |
| **アセットのバージョン管理** | GCS + Object Versioning | 過去バージョンを保持しロールバック可能 |

**CNAMEを使ったシンプルなアセット配信設定**:
```bash
# バケット作成（アジアリージョン）
bucket=gs://[game-assets-bucket]
gsutil mb -c STANDARD -l ASIA $bucket
gsutil defacl set public-read $bucket
# CNAMEで独自ドメインをGCSへ向ける
# CNAME: assets.example.com → c.storage.googleapis.com
```
