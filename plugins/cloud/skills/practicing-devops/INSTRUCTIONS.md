...
**詳細**: `implementing-observability` スキル、および [DATA-OBSERVABILITY.md](references/DATA-OBSERVABILITY.md) を参照

---

## ユーザー確認の原則

以下の判断分岐箇所では、必ずAskUserQuestionツールでユーザーに確認してください（本スキルの references/ 内の AskUserQuestion 節も同様に、AskUserQuestion が使えない環境（Codex CLI 等）では同じ選択肢を通常のテキスト質問として提示する）：
...
DockerとPodmanを使ったコンテナ管理の包括的ガイド。マルチステージビルド、セキュリティ強化、rootlessコンテナ、systemd統合まで網羅する。

### Docker

- **MCP基本操作**: `mcp__docker__list_containers()`, `start/stop_container()`, `get_logs()`, `deploy_compose()`
- **Dockerfileベストプラクティス**: マルチステージビルド（必須）、キャッシュ最適化（依存関係ファイルを先にCOPY）、非rootユーザー実行
- **Docker Composeパターン**: サービス定義、depends_on制御、named volume管理
- **セキュリティ強化**: distrolessベースイメージ、`USER 65532:65532`、`.dockerignore`

### Podman

- **三位一体**: Podman（コンテナエンジン）+ Buildah（イメージビルド）+ Skopeo（イメージ操作）
- **Dockerとの根本差異**: デーモンレス・rootless-first・ネイティブPodサポート・Quadletによるsystemd統合
- **rootless-first原則**: 常に一般ユーザー権限を第一選択。`-v /path:/path:Z`でSELinuxラベル付与
- **Kubernetes互換**: `podman generate kube <pod>` でK8s YAML生成

### 選択ガイド

| 要件 | 推奨 |
|------|------|
| 既存Dockerワークフロー継続 | Docker |
| セキュリティ重視・rootless必須 | Podman |
| K8s移行を想定 | Podman（Quadlet + YAML生成） |
| systemdサービス統合 | Podman Quadlet |

**詳細**: [CONTAINER-GUIDE.md](references/CONTAINER-GUIDE.md) - Docker/Podman統合ガイド全文
**Docker詳細**: [DOCKER-ENGINE.md](references/DOCKER-ENGINE.md), [DOCKER-SECURITY.md](references/DOCKER-SECURITY.md), [DOCKER-COMPOSE.md](references/DOCKER-COMPOSE.md)
**Podman詳細**: [PODMAN-ARCHITECTURE.md](references/PODMAN-ARCHITECTURE.md), [PODMAN-SECURITY.md](references/PODMAN-SECURITY.md), [PODMAN-SYSTEMD-KUBERNETES.md](references/PODMAN-SYSTEMD-KUBERNETES.md)
...
- `implementing-observability` - 監視システム設計・OpenTelemetry計装
...
- [CONTAINER-GUIDE.md](references/CONTAINER-GUIDE.md) - Docker/Podman統合コンテナ管理
