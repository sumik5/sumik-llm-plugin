...
`p{幅}`型の列は指定幅で自動折返しされるが、和文・欧文が混在する文章では「意味の区切り」ではなく「文字幅の限界」で折り返し位置が決まるため、単語の途中（例：「自動切替」が「自動切」「替」に分断される等）で不自然に折れることがある。統一感が必要な表では自動折返しに任せず、`\newline` で明示的に折る位置を指定するのが確実。

```latex
\begin{tabular}{lp{6cm}}
  項目 & 説明 \\
  \hline
  伝送速度 & 330/660/990\,kbps\newline{}を自動切替 \\
\end{tabular}
```

🔴 **重要な罠**: upLaTeX/pLaTeXで `\newline` の直後に全角文字（特にひらがな）が続くと、コマンド名の区切りが曖昧になり `! Undefined control sequence.` エラーになることがある。

```latex
330/660/990\,kbps\newlineを自動切替  % ← エラー: \newlineを が1つのトークンと誤認される
```

全角の開き括弧（`（`）が直後に続く場合はエラーにならないケースも確認したが、文字種によって挙動が変わり得るため、`\newline` の直後が全角文字になる場合は常に空グループ `{}` を挟んで安全にコマンド名を区切ること。

```latex
330/660/990\,kbps\newline{}を自動切替  % ← 安全
328\,kbps\newline{}（44.1\,kHz時）      % ← 同様に安全側で統一
```
