...
以下の判断が曖昧な場合は**必ずAskUserQuestionツールで確認**すること（AskUserQuestion が使えない環境では、同じ確認項目を通常のテキスト質問として提示する）:
...
**Web引用にはアクセス日（閲覧日）を明記する**のが実務上の慣習。また、ピリオド（`.`）の直後に`\url`等のコマンドを続ける場合は、通常のスペースではなく`\ `（バックスラッシュ+スペース）を使う。通常のスペースのままだと、ChkTeXが文末の広い間隔か単語間隔かを判別できずWarning 12を出すため、`\ `で単語間隔であることを明示する。

## 品質チェック（ChkTeX）

コンパイル確認に加え、ChkTeXで構文・組版上の問題を最終チェックする。

```bash
chktex -q document.tex
```

本スキルの出力で実際に発生しやすい警告と対処:

| Warning | 内容 | 対処 |
|---|---|---|
| 44 | `tabular`の縦罫線`\|`と`\hline`の使用 | `booktabs`の`\toprule`/`\midrule`/`\bottomrule`に置き換える（[表](#表)参照） |
| 12 | ピリオドの直後にスペースを挟んでコマンド（`\url`等）が続く | ピリオド直後は`\ `を使う（[参考文献](#参考文献)参照） |

🔴 **false positiveのため修正禁止**:

| Warning | 内容 | 対応 |
|---|---|---|
| 8 | `\url{...}`内のハイフンを範囲ダッシュの誤りと誤検知 | URLの中身は書き換えない（en-dash/em-dashに変えるとURLが壊れる） |
| 38 | `` ``...,'' ``のように閉じクォート直前にカンマがある | 句読点を閉じクォートの内側に置くのは正しい組版慣習（アメリカ式）。無闇に位置を変更しない |
