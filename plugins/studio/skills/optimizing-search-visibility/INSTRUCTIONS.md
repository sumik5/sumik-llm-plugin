...
| SEO記事本文を書く・コンバージョン文章術を使う | [`CONTENT-WRITING.md`](references/CONTENT-WRITING.md) | 執筆前リサーチ / 記事構成の型 / 離脱を防ぐ設計 / タイトル設計 |
| 運用の意思決定・診断・時系列ロードマップを立てる | [`OPERATIONS-DIAGNOSTICS.md`](references/OPERATIONS-DIAGNOSTICS.md) | 既存サイト資産の監査と意思決定 / コンテンツ剪定 / 実行ロードマップと運用サイクル |
...
| PREP法 | `PREP法`（Point/Reason/Example/Point） | answer-first・結論先出しの文章構成。AIが引用しやすい回答の型として `CONTENT-WRITING.md` で扱う |
| コンバージョン文章フレームワーク | `コンバージョン文章フレームワーク`（AIDA・QUEST・PASONA・BEAF等） | 読者を行動へ導く文章構成の型の総称。使い分けは `CONTENT-WRITING.md` |
| 4つのNOT | `4つのNOT`（Not open/read/believe/act） | 読者が離脱する4段階の心理的ハードルモデル。`CONTENT-WRITING.md` |
| MSP | `MSP`（Most Sustainable/Sharable Point。個の価値観・信念による差別化軸） | 執筆者固有の視点で差別化するテクニック。`CONTENT-WRITING.md` |
| 一次情報・二次情報・三次情報 | `一次情報`（自身の経験・実験）/ `二次情報`（一次情報の引用・要約）/ `三次情報`（又聞き・伝聞） | 情報の階層と信頼度。一次情報を高比率で配合することがE-E-A-Tの「経験」を裏付ける。`CONTENT-WRITING.md` |
| content pruning | `content pruning`（コンテンツ剪定） | 低品質・重複記事を削除/統合/リライト/noindexのいずれかで整理する運用判断。`OPERATIONS-DIAGNOSTICS.md` |
| ドメインパワー | `ドメインパワー` | 第三者ツールが算出する通俗的な指標であり、**Google公式のランキング指標ではない**。`OPERATIONS-DIAGNOSTICS.md`・`FOUNDATIONS-RANKING-EEAT.md` |
| エンティティ | `エンティティ`（発信主体の明示） | コンテンツの発信者・組織を検索エンジン/LLMが認識可能な形で明示する概念。`FOUNDATIONS-RANKING-EEAT.md` |
| カバレッジ率 | `カバレッジ率` | AEO固有KPI。想定質問セットのうち自社が言及/引用される割合。`MEASUREMENT.md` |
| 引用品質スコア | `引用品質スコア` | AEO固有KPI。AI応答内での自社言及の質（出典明記の有無・上位表示位置等）を評価する指標。`MEASUREMENT.md` |
...
3. コンテンツ強化
   ...
   ├─ 記事本文の執筆（構成・コンバージョン文章術・執筆前リサーチ） → CONTENT-WRITING.md
   └─ 既存資産の棚卸し・優先順位付け・content pruning判断 → OPERATIONS-DIAGNOSTICS.md
...
6. 計測・改善ループ
   ...
   └─ 診断結果を踏まえた時系列ロードマップ・リライトループの再設計 → OPERATIONS-DIAGNOSTICS.md
```
...
| 状況 | 誘導先スキル |
|------|------------|
| 文章のAI臭を除去・推敲したい | `writing-effective-prose`（`CONTENT-WRITING.md` は文レベルの推敲は扱わず、SEO記事本文の構成設計・執筆前リサーチに特化する） |
| マーケコピー・見出し・SNS投稿を生成したい | `studio:creating-content`（`CONTENT-WRITING.md` は短文マーケコピーではなく長文SEO記事の制作実務を扱う） |
