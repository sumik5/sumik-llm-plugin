...
SEOツール・コンサルタント費用をかけずに、汎用チャットLLMを「壁打ち相手の診断コンサル」として使うワークフロー。運用上の位置づけ（診断フローのどの段階で使うか・判断フレーム本体）は `OPERATIONS-DIAGNOSTICS.md` の「診断ワークフロー」を参照し、本節ではプロンプトパターンと注意点のみを扱う。

**自社認識確認プロンプト**
```
あなたはSEO/マーケティングの専門家です。[サイトURLまたは事業内容の説明]について、
第三者の視点で以下を教えてください:
- このサイト/事業は何を提供していると認識されるか
- ターゲット顧客層はどのような層だと推測されるか
- 競合と比べた際の強み・弱みとして推測されること
```

**競合URL比較プロンプト**
```
以下の自社ページと競合ページを比較してください。
自社: [自社URLまたは本文貼付]
競合: [競合URL1], [競合URL2]
比較観点: カバーしているトピックの網羅性、独自性のある情報の有無、
構成のわかりやすさ、信頼性シグナル（著者情報・データの有無）。
自社に不足している要素を具体的に指摘してください。
```

**文章問題点抽出プロンプト**
```
以下の文章をSEOライター・編集者の視点でレビューしてください。
[本文貼付]
指摘してほしい観点: 冗長な表現、結論が不明瞭な箇所、
検索意図とズレている可能性のある箇所、証拠・出典が不足している主張。
```

**注意点**
- 生成AIの回答は**壁打ち・仮説出しの補助**に留め、GSC/GA4等の実測値による裏取りなしに鵜呑みにしない（自社認識・競合比較はAIの推測であり事実確認ではない）
- 競合ページの本文を貼り付ける際は、著作権・利用規約の範囲内で行う（全文丸ごとの再配布・二次利用は避け、比較分析目的の引用に留める）
- 診断結果をそのまま施策に反映せず、人間が優先順位・実行可否を判断する（後述「人間の関与（Human-in-the-loop）」の原則）
...
- **OPERATIONS-DIAGNOSTICS.md**: 運用の意思決定・診断ワークフロー本体（生成AIを診断コンサルとして使う位置づけ）・時系列オーケストレーション
