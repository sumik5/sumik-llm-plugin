...
| (g) toolkit の不変バグ | `plugins/certificate/skills/creating-flashcards/scripts/anki_toolkit.py` の投入インフラ・HTML整形・冪等性で発見したコードレベルのバグ（ソース非依存・全セッション共通） |
...
- `plugins/certificate/skills/creating-flashcards/INSTRUCTIONS.md`
...
- `plugins/certificate/skills/creating-flashcards/references/ANKI-MCP-GUIDE.md`
- `plugins/certificate/skills/creating-flashcards/references/ANKI-MCP-TOOLS.md`
- `plugins/certificate/skills/creating-flashcards/references/SCRIPTS-CONTRACT.md`（scripts 投入インフラの契約定義・機械可読 CONTRACT ブロック）
- `plugins/certificate/skills/creating-flashcards/scripts/anki_toolkit.py`（投入インフラの実装本体。(g) の不変バグ判定時に既存実装を確認）
...
| 発見した知見 | 性質 | 振り分け先 | バンプ |
|------------|------|-----------|--------|
| `addNotes` の新しいエラー形式 / 冪等性の穴 / HTML整形の出力バグ | ソース非依存・不変 | `plugins/certificate/skills/creating-flashcards/scripts/anki_toolkit.py` を **Edit でコード修正** | PATCH |
...
- **(g) toolkit の不変バグ・(f) のうち投入インフラの不変な罠**は `plugins/certificate/skills/creating-flashcards/scripts/anki_toolkit.py` のコード修正へ。
...
### 知見1: [タイトル]
**対象ファイル**: plugins/certificate/skills/creating-flashcards/references/CONTENT-{DETECTION|BY-TYPE|COMMON}.md（知見の種別に応じて選択）
...
承認された知見を反映する。**振り分け判定表（Step 3）に従い**、不変バグは `plugins/certificate/skills/creating-flashcards/scripts/anki_toolkit.py` を Edit でコード修正し、ソース固有の知見は references へ散文追記する。編集後、書籍名・著者名・出版社名の混入がないか機械チェックする（scripts も対象に含める）:

```bash
grep -nE "『|』|TAC|オライリー|オーム社|技術評論|翔泳社|日経BP|インプレス" \
  plugins/certificate/skills/creating-flashcards/references/CONTENT-DETECTION.md \
  plugins/certificate/skills/creating-flashcards/references/CONTENT-BY-TYPE.md \
  plugins/certificate/skills/creating-flashcards/references/CONTENT-COMMON.md \
  plugins/certificate/skills/creating-flashcards/INSTRUCTIONS.md \
  plugins/certificate/skills/creating-flashcards/scripts/anki_toolkit.py \
  plugins/certificate/skills/creating-flashcards/scripts/parser_scaffold.py
```

検出ゼロを確認する。検出された場合は該当知見をユーザーへ返送し、汎用表現への置換を求める。

> 🔴 `plugins/certificate/skills/creating-flashcards/scripts/anki_toolkit.py` をコード修正した場合、`plugins/certificate/skills/creating-flashcards/references/SCRIPTS-CONTRACT.md` の CONTRACT ブロック（`<!-- CONTRACT:BEGIN -->`〜`END`）と実コードの整合を確認する。`QAPair` フィールド・公開API名を変えた場合は CONTRACT ブロックも同時更新する（契約は3箇所一致が必須）。`parse()` TODO や共通ヘルパの regex は自動修正しない。

### Step 6: バージョン更新・コミット・タグ付与

> ⚠️ Step 4 で「コミット・タグまで実行」が承認された場合のみ実施する

1. `plugins/certificate/.claude-plugin/plugin.json` の `version` を **PATCH bump**（パッチ番号 +1）
   - 理由: 既存スキルへの知見追記 = PATCH（新規コマンド追加の MINOR とは別の意味論）

2. Conventional Commits 形式でコミット（`git add` 対象に `plugins/certificate/skills/creating-flashcards/scripts/` を含める）:

   ```bash
   git add plugins/certificate/skills/creating-flashcards/ plugins/certificate/.claude-plugin/plugin.json
   git commit -m "docs(creating-flashcards): N件の追加知見を CONTENT-{DETECTION|BY-TYPE|COMMON}.md に反映"
   ```

   - 🔴 **`plugins/certificate/skills/creating-flashcards/scripts/anki_toolkit.py` をコード修正した場合**は、コミットメッセージを `fix(creating-flashcards): toolkit の <内容> を修正` にする（散文追記の `docs(...)` とは type を分ける）。散文追記とコード修正が混在する場合は分割コミットを推奨する。
