...
| 形式 | 変換方法 |
|------|---------|
| EPUB（テキスト） | `pandoc` CLIでMarkdown変換 |
| PDF（テキスト埋め込み） | `pdf-to-markdown` または `recognize-image-to-markdown`（pdftotextで直接） |
| 画像ベース＝スキャン本（PDF/EPUB/画像） | **第一選択: `scripts/ocr-apple-vision`（Apple Vision・高速 約1〜2秒/ページ）** ／ 構造重視時のフォールバック: `scripts/recognize-image-to-markdown`（VLM） |
| JSON | 構造化済みデータとして直接処理（変換不要） |

> 🔴 **スキャン本（画像ベースPDF/EPUB）はテキスト抽出では本文が取れない**。Kindle等の電子書籍は本文がページ画像で、`pdf-to-markdown`/`pandoc` は目次・奥付しか拾えない（実測: 529ページのスキャンPDFで抽出4KB・目次のみ・Q&A 0件）。OCRが必須。
>
> **OCRエンジンの使い分け（🔴 第一選択は Apple Vision）**:
> - **`scripts/ocr-apple-vision`（既定・高速）**: Apple Vision（Neural Engine）。約1〜2秒/ページとVLMの約12倍速。LLMでないため思考ログ漏洩・反復崩壊が原理的に起きない。テキスト＋座標を返し、列再構成で読み順を回復する。**まずこれを使う**。
> - **`scripts/recognize-image-to-markdown`（フォールバック・高忠実）**: ローカルVLM。約22秒/ページと遅いが、Markdownテーブル・見出し等の構造を文脈理解で再現する。**表や複雑レイアウトの忠実再現が品質に直結するソースのみ**こちらを使う。
>
> 判定法・列再構成・モデル選択・resume・既知の罠・トークン予算は [OCR-CONVERSION.md](references/OCR-CONVERSION.md) を必読。
...
> 使い捨て scaffold）に加え、`scripts/kentei_lab_import.py`（kentei-lab 収集）・
> `scripts/whizlabs_import.py`（whizlabs 収集）・`scripts/studying_import.py`
> （studying 収集）・`scripts/shikaku_drill_import.py`（shikaku-drill 収集）が、
> それぞれ Step 0 が生成する固定スキーマ JSON 専用の**恒久ブリッジ**として常設されている。これらの JSON は
...
### Step 0: 入力の判定（ファイルパス or URL）

引数が URL 形式（`http://` または `https://` で始まる）かファイルパスかを判定する。

- **URLでなければ** Step 1（ファイルパス処理）へそのまま進む。
- **URLの場合**、ホスト名で対象サイトを判定する。

| ホスト名に含まれる文字列 | 収集先 | 認証 |
|---|---|---|
| `kentei-lab.com` | kentei-lab 収集 | 不要 |
| `studying.jp` | studying 収集 | 必須（メール+パスワード） |
| `whizlabs.com` | whizlabs 収集 | 必須（メール+パスワード） |
| `shikaku-drill.com` | shikaku-drill 収集 | 不要 |

どれにも一致しない場合は未対応サイトである旨をユーザーに報告し処理を停止する（汎用URLスクレイピングへは
フォールバックしない。対応外サイトを収集ロジックにそのまま通すと壊れた結果を返すため、明示エラーの方が
安全）。

判定できたら [URL-COLLECTION.md](references/URL-COLLECTION.md) の該当サイト節を読み、以下を実行する。

1. 前提ツール確認（agent-browser・jq。認証必須サイトはログイン情報をAskUserQuestionまたは通常の対話で
   ユーザーに確認する。ハードコード・恒久保存はしない）
2. `${CLAUDE_SKILL_DIR}/scripts/collect-<site>.sh <url> [output-dir]` を実行する（大規模資格・コースは
   長時間になるため `run_in_background: true` を推奨。中断しても各サイト節のresume機構で再開できる）
3. 生成された JSON のパスを確認する
4. 生成 JSON をそのまま Step 1 の「〜収集済み JSON のファストパス」節（既存・変更なし）へ渡す

> 🔴 **`disable-model-invocation` との関係**: 本スキルは SKILL.md フロントマターで
> `disable-model-invocation: true` を持つため、URL 入力の場合も同一会話内で
> `Skill(skill: "certificate:creating-flashcards", ...)` を直接呼び出すことはできない（ユーザーが
> `/certificate:creating-flashcards <url>` を明示的に叩いた場合のみ起動可能）。同一会話内で URL 収集の
> 結果を直接ブリッジする必要がある場合は、収集して生成した JSON に対し、Step 1 の各ファストパス節に
> 記載された `${CLAUDE_PLUGIN_ROOT}` 実体パス解決手順（同一の回避手順）がそのまま適用される。新規の
> 重複記載はしない。

> studying の `scripts/collect-studying-choices.sh`（`choice_type: "multi_blank"`/`"fill_in_single"`
> 問題の選択肢〈語群〉後付け取得）は Step 0 の自動ディスパッチ対象に含まない。`collect-studying.sh`
> 実行後、該当 `choice_type` の問題が含まれる場合の追加手順として
> [URL-COLLECTION.md](references/URL-COLLECTION.md) の studying 節を案内する。

### Step 1: ファイル形式の検出と変換

引数として受け取ったファイルパスの拡張子を判定し、Markdownに変換する。

> 🔴 **まず「テキスト埋め込み」か「画像ベース（スキャン本）」かを判定する**。画像ベースはテキスト抽出では本文が取れない（目次のみ）。判定の決め手は「ページ数の割に抽出が極端に小さい」こと。運用詳細は [OCR-CONVERSION.md](references/OCR-CONVERSION.md) を参照。

**EPUB → Markdown:**

```bash
# テキストEPUB: pandoc（ファイル名はソース由来のユニーク名にして衝突回避）
pandoc -t markdown -o /tmp/<descriptive-name>.md <input.epub>
# 画像ベースEPUB（スキャン本）— 第一選択: Apple Vision（高速）
${CLAUDE_SKILL_DIR}/scripts/ocr-apple-vision <input.epub-images-dir/> -o /tmp/<descriptive-name>.md
#   ※ EPUB は画像を展開してディレクトリOCR、または構造重視なら recognize-image-to-markdown(VLM)
```
...
```bash
# 🔴 第一選択（スキャン本）: Apple Vision OCR（高速・約1〜2秒/ページ・思考ログ漏洩なし）
${CLAUDE_SKILL_DIR}/scripts/ocr-apple-vision <input.pdf> -o /tmp/<descriptive-name>.md
...
> 🔴 **巨大OCR出力はReadツールで全文読みしない（トークン予算）**: スキャン本1冊は 10〜20万トークン規模になりうる。`Read` 全文読みは（fork 子コンテキストでも）溢れさせる。`wc -l`・`grep -n`（セクション境界マーカー）・`sed -n`（該当領域サンプリング）で**構造だけ把握**し、全件抽出は `parse()`（Python側・全文を処理）に委ねる。全文Readは小さなテキスト変換結果（数千トークン）に限る。
...
Step 0（URL収集・kentei-lab）が出力した JSON（トップレベルに `exam_title`・`slug`・`questions`
...
3. Step 5（デッキ・ノートタイプ選択）へ直行する。デッキ選択（Step 5a）では `exam_title` から算出した
   `検定試験::<検定名>::<級>::kentei-lab`（級を検出できない試験は `検定試験::<検定名>::kentei-lab`）を
   「推奨」の先頭選択肢として提示する。ノートタイプ選択（Step 5b）は通常どおり
...
> `--deck` を省略すると `default_deck_name` が上記形式を自動算出する（級を検出できない試験は
> `検定試験::<検定名>::kentei-lab` になる）。
...
> 🔴 **`disable-model-invocation` との関係**: この制約と回避手順は Step 0 冒頭の説明（L135-141付近）を参照。

#### whizlabs 収集済み JSON のファストパス（AI 構造推測をスキップ）

Step 0（URL収集・whizlabs）が出力した JSON（トップレベルに `course_title`・`course_url`・`quiz_title`・
...
> ⚠️ **MCMR（複数選択）の `correct` 抽出は実例未確認**: Step 0（URL収集・whizlabs）の収集スクリプト側で "Correct Answer: A, C"
...
Step 0（URL収集・studying）が出力した JSON（トップレベルに `course_title`・`course_url`・`category`・
...
> ⚠️ **studying は著作権への配慮が必要なプラットフォーム**: Step 0（URL収集・studying）が収集した問題文・
> 解説の実例を、投入作業中の会話・コミットメッセージ・スキル本文に引用しない（ユーザー自身の Anki 個人
> 利用の範囲でのみ投入する）。

#### shikaku-drill 収集済み JSON のファストパス（AI 構造推測をスキップ）

Step 0（URL収集・shikaku-drill）が出力した JSON（トップレベルに `exam_title`・`slug`・
...
> 🔴 **資格名収集元は投入前にデッキ名の表記ゆれを突合すること**: kentei-lab・whizlabs・studying・
> shikaku-drill のいずれも、`exam_title`/`course_title` 等から自動算出したデッキ名をそのまま使うと、
> 同一資格を別ツール・別サイトから収集した際の表記ゆれ（中黒の有無、全角/半角、略称/正式名称の差）で
> Anki デッキツリーが分裂するリスクがある。実例: kentei-lab 収集で観測した「ファイナンシャルプランナー
> 技能検定」と既存デッキ「ファイナンシャルプランナー」の級表記ゆれ、shikaku-drill 収集
> （メンタルヘルス・マネジメント検定Ⅰ種）で観測した中黒あり「メンタルヘルス・マネジメント検定」と
> 既存デッキ「検定試験::メンタルヘルスマネジメント検定::II種::kentei-lab」（中黒なし）の表記ゆれ。
> **対策**: 投入前に必ず AnkiConnect の `deckNames` を確認し、同一資格の既存デッキが見つかった場合は
> 表記を突合する。食い違いがあれば、収集した生 JSON は無加工のまま保持しつつ、投入専用の正規化コピー
> （`exam_title` 等の表記だけを既存デッキ系列に揃えた複製ファイル、例: `<slug>.anki-import.json`）を
> 作成してから投入する。既存デッキが見つからなければ自動算出のまま進めてよい。
...
> ⚠️ **複数分類が存在するデータ形式は1形式の欠落発見だけで対応を終わらせない**: 例えば `choice_type`
> （boolean/single/multi_blank/fill_in_single/unknown 等）のように分類が複数あるデータで、ある1形式に
> Question 面の情報欠落（選択肢・語群等）を発見した場合、残りの全形式についても同じ種類の欠落がないか
> 実機で一括点検してから実装依頼を出す。1形式だけを見て「他の形式は現状のフィールドで足りるはず」と
> 類推すると、別形式に固有の異なるマークアップ（実例: `multi_blank` 形式の語群は `h3+ul` パターンだった
> が、`fill_in_single` 形式の語群は全く別の `ul.ipt-button > li.notosans-mark` ボタン形式だった）を
> 見逃し、同じ調査・実装・検証サイクルを再度繰り返す手戻りが生じる。
...
> ⚠️ **AskUserQuestion 使用不可環境（background 実行）での振る舞い**: このスキルを background agent として起動した場合（`run_in_background: true` 等）、`AskUserQuestion` ツールが呼び出せないことがある。この場合は確認必須ステップ（サンプル確認・デッキ選択・ノートタイプ選択等）を無断で自動決定せず、**実行を確認ポイント手前で止め**、完了報告に (1) 未確定の判断点とその推奨案、(2) サンプルQ&Aの実データ、(3) 再開に必要な具体コマンド、をすべて書き出して呼び出し元（本体）へ差し戻す。本体側が実データを直接検証し `AskUserQuestion` で確認したうえで投入を再実行する二段構成が機能する。
