...
${CLAUDE_SKILL_DIR}/scripts/pdf-to-markdown "<input.pdf>" /tmp/probe.md && wc -lc /tmp/probe.md
```
...
> ⚠️ **短い選択肢の近接2列配置による列判定破綻**: 組み合わせ問題（サブ項目a〜e＋短い組合せ選択肢ア〜エ）や空欄穴埋め型では、選択肢テキストが短いため紙面上で複数列に近接配置され、Vision の列再構成ヒューリスティック（x座標ギャップでの2分割）が誤動作する。空欄穴埋め型では3つ目以降の空欄候補が離れた位置に配置され、読み順が説明文の後ろまでずれることもある。表・2D図の崩壊とは別の失敗モードで、該当ページのみVLM再OCRしても座標由来の崩壊は再現されうる。復旧は解説文中の確定的記述（「正解はアとなる」等）を照合の一次情報源にする。実観測: 資格試験問題集200問のうち12問がこの崩壊で選択肢テキスト不完全のまま抽出され、解説文照合による手動相当の復元を要した。
