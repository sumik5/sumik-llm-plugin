...
### 一発勝負構造コース（模擬試験・診断テスト等）の収集（手動・オプトイン専用フロー）

🔴 **重要な注意（本節の手順を実行する前に必ず読むこと）**: 模試モードや診断テストのような一発勝負構造の
コースは多くの場合「最初の1回だけ」しか実行できず、収集のために最後まで解答させることは**ユーザー本人の
実受験権を取り消し不可能な形で消費する**。診断テストであっても模擬試験と同様にこの警告がそのまま適用され、
弱めてはならない。そのため以下を徹底する。

- 本フローは`scripts/collect-studying.sh`の自動収集ループには**絶対に組み込まない**（手動・オプトイン
  専用の独立手順として扱う）。
- 実行する場合は、その都度**ユーザーへ明示的に確認し、同意を得てから**行う。確認時は「対象の模擬試験・
  診断テスト名」「消費される受験権の内容（例: 残り受験回数が1回のみである・受験後は年度データ更新まで
  再受験不可である等）」を具体的に伝えたうえで判断してもらう。
- 一度実施したら元に戻せないことを、実行前に必ずユーザーへ伝える。

🔴 **検出条件（実機確認済み）**:

- `category`が「直前期」等、かつ問題数が通常科目より大幅に多い（例: 20問・60問等の長時間試験）
  セクションは模擬試験（一発勝負の「模試モード」）の可能性が高い。この種のセクションは、通常科目と
  同じ手順で解説一覧ページ（`course/practice/list/id/<practice_id>/a/on/`）へ直接アクセスしても
  **404**を返す（模試モードで最後まで解答し採点を完了させるまで解説一覧ページが生成されない仕様であり、
  `collect-studying.sh`側の不具合ではない）。
- 🔴 **`category`表記が「直前期」を含まない独立コースでも同種の構造がありうる（実機確認済み）**:
  コースページに科目一覧・トグル展開が無く「受講を開始する（無料）」ボタンのみが存在する構造であれば、
  同種の一発勝負構造の可能性が高い。実機確認済み事例:
  `https://member.studying.jp/course/id/2693/`「FP3級試験 実力診断テスト(2026)」。全18問
  （3問×6科目）、受講期限内で1回のみ受験可、受験後は新年度データ更新まで再受験不可・受験前の
  総合結果レポートも消失する注意書きがある。

🔴 **手順（模擬試験の場合・実機確認済みの手動フロー・ユーザー同意取得後にのみ実行）**:

1. `course/practice/index/id/<practice_id>/f/0/`を開く。
2. 「模試を開始」ボタン（`.practice-mode-btn.mode-3`）をクリックする。
3. 各設問で選択肢（`li.notosans-mark`）を1つクリックする。クリックすると自動採点・自動で次の設問へ
   遷移する（Whizlabsのような「Show Answer」ボタンや「Next」リンクのクリックは不要）。
4. 全問終了後に表示される「終了確認」ページで「終了して採点する」をクリックする。
5. 結果ページに「解説一覧表示」リンクが出現する。以後は通常科目と同じ`collect-studying.sh`の抽出
   ロジック（`.list_question`/`.list_answer`等）がそのまま使え、解説一覧ページ
   （`course/practice/list/id/<practice_id>/a/on/`）から通常どおり問題・正解・解説を取得できる。

正解ラベルが「１」「２」「３」等の数値表記の場合、既存の`choice_type`判定優先順位により
`fill_in_single`（`.notosans-mark`の値が○×以外の値であるため）に分類される。これは既存ロジックの
正しい挙動であり、模擬試験向けの新規対応は不要。

#### 診断テストの解説一覧到達経路（模擬試験との違い・実機確認済み）

診断テスト（例: 上記「FP3級試験 実力診断テスト」）は模擬試験と異なり、「終了して採点する」から
解説一覧ページへワンステップで到達しない。以下の4段階の遷移を経る必要があった。

1. 全問解答後、「終了して採点する」で採点確定 → `course/practice/gross/id/<gross_id>/` の
   **総合結果レポート**ページに遷移する（スコア・科目別バランスチャート・得意/苦手科目が表示される）。
2. 総合結果レポートページには科目ごとに「問題と解説」リンクが並んでいる（今回は6科目分）。1つクリック
   すると `course/practice/score/id/<score_id>/` の**科目別結果詳細**ページに遷移する（この`score_id`は
   科目ごとに異なる）。
3. 結果詳細ページ内に「解説一覧表示」リンクがあり、クリックすると
   `course/practice/list/id/<practice_id>/a/on/` の解説一覧ページに到達する（模試と同じ、通常の
   `collect-studying.sh`抽出ロジックがそのまま使える構造）。
4. 実機では6科目の`practice_id`が連番だった（例: 218213, 218214, 218215, 218216, 218217, 218218）。
   1科目目の`practice_id`さえ判明すれば、残りは連番として直接`open`を試す効率化ができる。
   🔴 **ただし連番が保証される仕様上の裏付けはない**ため、必ず1科目目はUIクリックで到達し実際のIDを
   確認したうえで、後続科目は連番仮説を都度検証しながら（実際にページを開いて期待した科目の内容か
   確認しながら）進めること。
