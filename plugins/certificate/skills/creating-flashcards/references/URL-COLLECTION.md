...
### 「直前期」模擬試験の収集（手動・オプトイン専用フロー）

🔴 **重要な注意（本節の手順を実行する前に必ず読むこと）**: 模試モードは多くの場合「最初の1回だけ」しか
実行できず、収集のために模試を最後まで解答させることは**ユーザー本人の実受験権を取り消し不可能な形で
消費する**。そのため以下を徹底する。

- 本フローは`scripts/collect-studying.sh`の自動収集ループには**絶対に組み込まない**（手動・オプトイン
  専用の独立手順として扱う）。
- 実行する場合は、その都度**ユーザーへ明示的に確認し、同意を得てから**行う。確認時は「対象の模擬試験名」
  「消費される受験権の内容（例: 残り受験回数が1回のみである等）」を具体的に伝えたうえで判断してもらう。
- 一度実施したら元に戻せないことを、実行前に必ずユーザーへ伝える。

🔴 **検出条件（実機確認済み）**: `category`が「直前期」等、かつ問題数が通常科目より大幅に多い
（例: 20問・60問等の長時間試験）セクションは模擬試験（一発勝負の「模試モード」）の可能性が高い。
この種のセクションは、通常科目と同じ手順で解説一覧ページ（`course/practice/list/id/<practice_id>/
a/on/`）へ直接アクセスしても**404**を返す（模試モードで最後まで解答し採点を完了させるまで解説
一覧ページが生成されない仕様であり、`collect-studying.sh`側の不具合ではない）。

🔴 **手順（実機確認済みの手動フロー・ユーザー同意取得後にのみ実行）**:
