...
更新をバッファリングすることで、後続の同じノードへの書き込みでI/O要求を削減する方式。更新に適したメモリ構造を使用して、更新をバッファリングし、遅延を伴ってそれらを伝播させる。
...
- ベースノードとデルタノードのサイズはページと揃うことはほとんどないので、それらを連結して格納することは理にかなっている
...
- ツリーを高さの中間レベルで分割し、各サブツリーを同じ方法で再帰的に分割する
