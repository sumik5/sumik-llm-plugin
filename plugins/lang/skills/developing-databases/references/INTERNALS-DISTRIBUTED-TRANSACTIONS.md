...
- **準備フェーズ中の障害**: 準備メッセージを受け取ったノードと受け取っていないノードが混在しうるため、スプリットブレインを引き起こすおそれがある
...
1. **読み取りセットと書き込みセットを分析**: トランザクションが必要とするデータレコードを特定し、アクティブな参加ノード（書き込みセットの要素を保持している）のリストを作成
2. **ローカルデータを取集**: トランザクション実行のために必要なローカルデータを収集
3. **アクティブな参加ノードで実行**: ワーカースレッドがアクティブな参加ノードで実行される場合、他のノードから転送されたデータレコードを受け取る
4. **結果をローカルストレージに永続化**: 実行結果を他のノードに転送し、結果をそれぞれローカルに永続化
...
| **パフォーマンス** | どの処理がどのエポック/バッチに入るかについてシーケンサが確実に合意できるようにする必要があり、障害が発生した場合にはPaxosを通じてリーダーの状態をノードが再生成する必要がある |

**メリット**:
- 実行フェーズでの調整オーバーヘッドがなくなり、すべてのレプリカが同じ入力を受け取る
- その結果、すべてのレプリカが同等の出力を生成するようになる

**デメリット**:
- 処理が発生するまでに遅延が生まれる（エポックの境界）
- トランザクションを決定論的に実行できる形にする必要がある
...
- どのレプリカでも実行でき、最新のタイムスタンプでの読み取りが可能
- 特定のタイムスタンプでの読み取りの場合、そのタイムスタンプまでの書き込みを反映済みの任意のレプリカで読み取りを実行できる
...
- トランザクションのタイムスタンプは、分散トランザクションの場合でも直列化可能性と同等のリアルタイムの特性を持つ
- T1がT2の開始前にコミットする場合、T1のタイムスタンプはT2のタイムスタンプより小さくなる
- これはすべての読み取りとすべての書き込みに適用される

### 2PCとTrueTimeの統合

Spannerは、一貫性のあるトランザクションログのレプリカを持つPaxosグループを基盤とし、シャード内の一貫性を保証するためのロックテーブルと、複数シャードにまたがる分散トランザクションを管理するトランザクションマネージャーを使用して2PCを実行し、トランザクションの順序付けにTrueTimeを使用：

1. **グループリーダーは一貫性を確保するために協調**: ロックテーブルへの問い合わせを行なって、トランザクションの順序付けとシャード内の一貫性を保証する
2. **2PCとTrueTimeの統合**: クロスシャードのトランザクションでは、Paxosグループのリーダー間で2PCを実行し、TrueTimeのタイムスタンプでコミット順序を確定する
3. **パーティション化された分散データにおけるトランザクションの順序付け**: このアプローチにより、パーティション化された分散データにおいてもトランザクションの実行順序を定めることが可能になる
...
| 方式 | 説明 | メリット | デメリット |
|------|------|---------|----------|
| **レンジベース** | データをキーの範囲で複数に分割し、範囲ごとにノードへ割り当てて管理 | 範囲クエリが効率的 | ホットスポットが発生しやすい |
...
- **仮想ノード**: リング上に各ノードごとに（リソースに応じて）複数の仮想ノードを配置し、各仮想ノードは前の仮想ノードの位置と自身の位置の間にある値の範囲を管理
- **リバランス**: ノードが追加されたり削除されたりしても、影響を受けるのは隣接する範囲のみで、再配置されるキーの数が最小化される

**特徴**:
- もっとも単純な代替方式では、ハッシュ値に対してクラスタサイズによるモジュロ（つまり余り）を取ります
- システム内にN個のノードがある場合、ターゲットとなるノードIDは、hash(v) modulo Nを計算して求める
- このアプローチの主な欠点は、ノードが追加されたり削除されたりした場合、クラスタサイズがNからN'に変更されるために、hash(v) modulo N'から返される多くの値がもとの値と異なる点

**コンシステント**という用語は、K個の取り得るハッシュキーとN個のノードを持つシステムにおいて、ハッシュテーブルのサイズ（ノード数）が変更されるときに、再配置が必要になるキーが平均でK/N個に抑えられる、ということを指します
...
- **読み取りスナップショット**: トランザクションの開始タイムスタンプより前にコミットされたすべての値が含まれます
- **書き込みと書き込みの競合**: 複数のトランザクションが同じセルに対して書き込みを行おうとするケースでは、どちらか1つのみがコミットします（通常は最初にコミットしたほう）
- **アノマリー**: スナップショット分離では、書き込みスキュー（write skew）と呼ばれるアノマリーを防げません
...
- ロックの1つはプライマリロックとされ、残りはセカンダリロックになります
- **衝突の可能性**: 競合の可能性があるかどうかを確認し、他のトランザクションがより新しいタイムスタンプを持つデータをすでに書き込んだかどうか、あるいはタイムスタンプにかかわらず、未解放のロックがあるかどうかを確認

競合が検出された場合、トランザクションは中断します。

#### c) commit

競合の可能性がないと判断された場合、トランザクションは処理を続行します。第2フェーズでは、プライマリロックから始めて、クライアントはそのロックを解放し、最新のデータポイントのタイムスタンプで更新した書き込みメタデータのレコードに置き換えます。
...
| **コミット済みメタデータ** | コミットされたデータポイントの情報（書き込みメタデータ）とロックを、それぞれ異なる列に格納します |
| **アノマリー** | スナップショット分離のため、書き込みスキューは防げない |

---

## 調整の回避

分散トランザクションのパフォーマンスとスケーラビリティを向上させるために、調整の回避（coordination avoidance）を試みる手法があります：

### I-Confluence（不変合流性）

不変条件を維持したまま、分散した2つのデータベース状態を1つにマージできることを保証する特性として定義されています。この場合の不変条件（Invariants）は、ACIDにおける一貫性（C）に対応します：

- **任意の2つの有効な状態はマージして1つの有効な状態に変えられる**ので、I-Confluentな操作は、逐次の調整処理なしで実行できます
- その結果、パフォーマンスの特性とスケーラビリティの向上につながります

### RAMP（Read Atomic Multi-Partition）トランザクション

マルチパーティションの同時実行制御と現在実行中の操作のメタデータを使用して、欠落している状態の更新を他のノードから取得し、読み取りと書き込みの操作を並行実行します：

- **Read Atomicの分離レベル**: トランザクションが、実行中・未コミット、または中断されたトランザクションからの、処理中の状態変更を見ることができない
- **fractured read**: トランザクションに、他のトランザクションによって実行された書き込みのサブセットのみが見えてしまう現象
- **分離レベル**: Read Atomicの分離レベルでは、fractured readも発生しません
