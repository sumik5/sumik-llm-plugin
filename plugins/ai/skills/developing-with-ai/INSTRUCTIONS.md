...
- `references/DIALOGUE-PATTERNS.md`: LLM対話パターン（8パターンの詳細・選択フロー）
- `references/AUGMENTED-PRACTICES.md`: チーム実践・ワークショップ・アジャイル拡張
- `references/DESIGN-PATTERNS-WITH-LLM.md`: デザインパターンのLLM活用（GoF拡張・新興パターン）
- `references/ROLE-SHEETS.md`: TDP詳細方法論・役割別シート（PO/Dev/Coach/Manager）
...
> AskUserQuestion が使えない環境（Codex CLI 等）では、同じ選択肢を通常のテキスト質問として提示して回答を待つ。
...
| [`DIALOGUE-PATTERNS.md`](references/DIALOGUE-PATTERNS.md) | LLM対話パターン（Socratic Questioning, Guided Exploration, Reverse Specification等 8パターン・選択フロー・プロンプト集） |
| [`AUGMENTED-PRACTICES.md`](references/AUGMENTED-PRACTICES.md) | チーム実践・5ワークショップ設計・アジャイルリチュアル拡張・拡張開発マニフェスト |
| [`DESIGN-PATTERNS-WITH-LLM.md`](references/DESIGN-PATTERNS-WITH-LLM.md) | GoF/アーキテクチャパターンのLLM拡張プロンプト・7つの新興パターン |
| [`ROLE-SHEETS.md`](references/ROLE-SHEETS.md) | Test-Driven Prompting詳細・ツールシート・役割別拡張シート（PO/Dev/Coach/Manager） |

---

## LLM対話設計

ソフトウェア設計においてLLMとの対話を体系的にデザインするための方法論。プロンプトの「書き方」を超え、対話の「設計」に焦点を当てます。

### Grammar of Intention（意図の文法）

LLMとの対話を構成する5つの基本ジェスチャー:

| ジェスチャー | 目的 | 使用場面 |
|-------------|------|---------|
| **Frame** | 文脈と制約を設定する | 対話の開始時、スコープの定義 |
| **Ask** | 明確な質問で情報を引き出す | 要件の深堀り、選択肢の探索 |
| **Rephrase** | 理解を言い換えて確認する | 認識のずれを検出、共有理解の構築 |
| **Synthesize** | 複数の情報を統合する | 議論のまとめ、設計判断の統合 |
| **Test** | 仮説や提案を検証する | エッジケースの検証、設計の堅牢性確認 |

### 8つの対話パターン（クイックリファレンス）

| # | パターン | 状況 | 核心 |
|---|---------|------|------|
| 1 | **Socratic Questioning** | 要件が曖昧 | 問いの連鎖で本質的な要件を引き出す |
| 2 | **Guided Exploration** | 問題が複雑 | 段階的に解決空間を探索する |
| 3 | **Reverse Specification** | レガシーコード理解 | 既存コードから仕様を逆推論する |
| 4 | **Mirror Model** | 選択肢比較 | LLMの理解を鏡のように映し返す |
| 5 | **Clarification by Counterexample** | 提案の限界検証 | 反例で境界条件を明確化する |
| 6 | **Test-Driven Prompting (TDP)** | テスト可能な設計 | テスト基準を先に定義してからプロンプトを設計 |
| 7 | **Visual Reformulation** | 概念の可視化 | 抽象概念を視覚的表現に変換する |
| 8 | **Systemic Care** | 根本原因分析 | システム全体の健全性を診断する |

**詳細:** `references/DIALOGUE-PATTERNS.md` 参照

### チーム導入とワークショップ

LLM対話パターンをチームに導入するための段階的アプローチと5つのワークショップ設計。

**詳細:** `references/AUGMENTED-PRACTICES.md` 参照

### デザインパターンのLLM活用

GoF/アーキテクチャパターンの適用をLLMで拡張するプロンプト戦略と、7つの新興パターン。

**詳細:** `references/DESIGN-PATTERNS-WITH-LLM.md` 参照

### TDP・役割別シート

Test-Driven Prompting の詳細方法論と、PO/開発者/アジャイルコーチ/マネージャーそれぞれの役割に応じた対話パターン活用ガイド。

**詳細:** `references/ROLE-SHEETS.md` 参照
...
- `creating-content`: AIコピーライティング・クリエイティブ制作
- `researching-libraries`: ライブラリ調査（AI支援開発前の事前調査必須）
- `product:practicing-product-management`: PM向けClaude Code活用（コードベース調査・フィードバック分析・競合分析・要件生成）
