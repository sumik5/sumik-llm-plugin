...
### ストリーミングアプリのランタイム選定（Edge vs Serverless）

ストリーミングAIアプリでは、デプロイ先のランタイム選定が長時間ストリームの信頼性を左右する。`maxDuration = 30` を設定していても、ランタイムの特性を理解していないとストリームが途中で切れる原因になる。

| ランタイム | 最大実行時間 | 初回応答の制約 | 無限ストリーム | 向いているケース |
|-----------|------------|-------------|-------------|----------------|
| **Edge Functions** | 上限なし | 初回応答を概ね**25秒以内**に返す必要がある | 可（初回応答後は継続してストリーム可能） | 長時間の生成、低レイテンシが必要なチャット |
| **Serverless Functions** | ハード上限あり（`maxDuration` の範囲内） | 制約なし | 不可（実行時間上限で打ち切られる） | 短〜中程度の応答、Node.js専用APIに依存する処理 |

> **なぜ長いストリームが切れるか**: Serverless Functions は実行時間にハード上限があるため、上限を超えるストリームは強制的に打ち切られる。Edge Functions は実行時間の上限こそ無いが、初回チャンクを25秒以内に返さないとストリーム自体が確立しない。`maxDuration` の指定だけではこの差は吸収できない。

> **判断材料**: 生成が長引くチャットや段階的な応答を返すアプリは Edge ランタイムを選び、初回チャンクを早期に流す実装にする。逆に Node.js 固有のAPIやライブラリに依存する処理は Serverless を選ぶが、その場合は応答全体が `maxDuration` 内に収まる設計にする。ランタイムごとの細かな仕様はデプロイ先のドキュメントを確認する。
...
コマンドラインからVercelへデプロイする標準的な手順:

**プリデプロイチェック（必須）**

- **Node.jsバージョンの確認**: Vercelダッシュボードの「Build & Deployment」設定で、ローカル環境と一致するNode.jsバージョン（18.x等）を指定する
- **ローカルビルドの検証**: `npm run build` をローカルで実行して成功することを確認する。ローカルでビルドが失敗するとVercel上でも必ず失敗する

**デプロイ手順**

```bash
# 1. Vercel CLIをグローバルにインストール
npm install -g vercel

# 2. プロジェクトディレクトリでデプロイを実行
vercel
```

初回実行時はVercelアカウントへのログイン・プロジェクト設定が対話形式で行われる。以降は `vercel --prod` で本番環境へ直接デプロイできる。
