...
### 長文要約のチェーン戦略選定（stuffing / MapReduce / refine）

検索したチャンクを最終回答に統合する場面や、コンテキストウィンドウに収まらない長文ドキュメントを要約する場面では、複数チャンクをどう要約チェーンに流すかの戦略選定が品質とコストを左右する。広く使われる3戦略にはそれぞれ明確な得意・不得意がある。

| 戦略 | 仕組み | 得意 | 不得意・リスク |
|------|--------|------|--------------|
| **stuffing** | 全文を1回の呼び出しでプロンプトに詰め込み一括要約 | コンテキストウィンドウに収まる短文。最も単純・低コスト | 長文・複雑文書はウィンドウ制限を超えて破綻する |
| **MapReduce** | 各チャンクを独立要約（map）→ 中間要約を集約（reduce） | チャンク要約を並列化できる。大規模文書を分割処理 | 全チャンクをループするため、極端に大きい文書では呼び出し回数が嵩み非効率 |
| **refine** | 先頭チャンクの初期要約を、後続チャンクで逐次更新（直列） | 前段の文脈を引き継いで後段で改善できる。文脈依存の強い文書 | 実装が複雑。初期要約の品質が低いと、その劣化が後段に伝播する |

**選定軸**：

- **短文（ウィンドウに収まる）** → stuffing（単純さ優先）
- **大規模で並列化できる** → MapReduce（mapを並列化してスループット確保。ただし超大規模では呼び出しコストに注意）
- **後段の文脈で前段を改善したい（連続性重視）** → refine（直列だが文脈保持に強い）

> MapReduce も refine も結局は全チャンクをループするため、極端に大きい文書では依然コストが膨らむ。その場合は要約前に k-means クラスタリングや埋め込みベースのチャンク選別で「代表チャンクのみ要約する」前処理を挟み、入力量そのものを削るアプローチが有効。
