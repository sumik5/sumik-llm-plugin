...
### LLM間のツール定義スキーマ差と境界設計

ほぼすべての現代 LLM が Tool Calling をサポートするが、ツール定義スキーマはプロバイダ間で分岐する。

| スキーマ形式 | 概要 |
|------------|------|
| `input_schema` キー配下 | あるプロバイダが採用する形式（引数を直下に列挙） |
| `parameters` キー配下 | 別のプロバイダが採用する形式 |
| `function` キー配下の `parameters` | さらに別のプロバイダが `function` オブジェクトにネストする形式 |

**境界設計の原則**: provider 固有のスキーマをアプリ全体に漏らさない。3つの実装レイヤー（低レベル API・LLM 非依存フレームワーク・MCP）のいずれかで境界化し、アプリ本体への流出を防ぐ。低レベル provider client 自体は禁止ではなく、adapter 層の内側に閉じ込めて LLM 乗り換え時の変更点をその層に局所化するなら許容できる。LLM を複数回乗り換えるプロジェクトほど、スキーマ境界設計の効果が大きい。
