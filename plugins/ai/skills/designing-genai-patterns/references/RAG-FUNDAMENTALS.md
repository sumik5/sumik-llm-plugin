...
### RAG か、大コンテキスト窓への全文投入か

文書がモデルのコンテキスト窓に収まる規模であれば、検索（retrieval）を省いて**全文をプロンプトへ直接投入**する選択肢がある。チャンキングや検索に起因する誤りを回避でき、実装もシンプルになる。

反復クエリを想定する場合は **Prompt Caching（P25）** と組み合わせ、文書のトークン処理を再利用してコストを抑えるのが定石。

| 観点 | 全文投入（P25 と組み合わせ） | RAG（P6–P8） |
|------|---------------------------|------------|
| **実装コスト** | 低い（パイプライン不要） | 高い（チャンキング・インデックス構築が必要） |
| **1クエリあたりの入力コスト** | 全文分を毎回消費（P25 キャッシュで軽減） | 検索チャンク分のみ消費 |
| **更新対応** | 都度最新文書を渡せば常に最新 | インデックス更新スケジュールに依存 |
| **文書規模** | コンテキスト窓に収まる文書のみ | 任意規模（大規模は P8 Indexing at Scale） |
| **鮮度・更新頻度が高い場合** | 毎回全文を渡すコストが増大しやすい | RAG が優位 |

近年のモデルは 100 万〜200 万トークン級の窓を持つが、全文投入のコスト（入出力合算課金）と更新頻度の高さでは依然 RAG が優位な場面が多い。「文書が小さい・更新が少ない・窓に収まる」ならまず全文投入＋P25 を試し、規模や更新頻度が上がれば P6 以降の RAG へ移行する段階的アプローチが実用的。
...
### 埋め込みモデル選定の落とし穴（RAG事故防止）

埋め込みベクトルは「どのモデルが・どんなアーキテクチャで・どんなデータで学習したか」に固有である。異なるプロバイダ・異なる版で生成したベクトルを同じインデックスに混在させると、各ベクトルが捉える意味空間がズレるため類似度計算が不正確になり、検索が静かに壊れる（エラーは出ず、関連しないチャンクが返る）。RAGで頻発する事故なので、設計段階で次の2点を契約として固定する。

**1. コーパスとクエリは必ず同一の埋め込みモデルで埋め込む**

インデックス時のチャンクと検索時のクエリを別モデルで埋め込むと、両者のベクトルが比較不能になる。クエリ埋め込みは「保存済みベクトルと同じモデル・同じ版」で生成することを保証する。

```python
# ❌ 事故：インデックスとクエリで別プロバイダのモデルを使う
index_vec = openai_embed(chunk)        # OpenAI の意味空間
query_vec = local_model_embed(query)   # 別の意味空間 → 類似度が無意味

# ✅ 正解：同一モデル・同一版を共有
EMBED_MODEL = "text-embedding-3-small"  # コーパス・クエリ・再インデックスで固定
index_vec = embed(chunk, model=EMBED_MODEL)
query_vec = embed(query, model=EMBED_MODEL)
```

**2. 埋め込みモデルは版単位で決定論的 → 版を変えたら全件再インデックス**

テキスト生成モデルは同一入力でも確率的に出力が揺れるが、埋め込みモデルは**版が同じなら同一入力に対して常に同一ベクトルを返す（決定論的）**。逆に言うと、版が上がる・学習データやアルゴリズムが更新されると、同じ入力でもベクトルが変わる。新旧の版が混在したインデックスは比較不能になるため、**版を変えるときは保存済みベクトルを全件（または差分で段階的に）再生成する**（P8 Indexing at Scale のモデルライフサイクル管理を参照）。

| 性質 | テキスト生成モデル | 埋め込みモデル |
|------|------------------|--------------|
| 同一入力への出力 | 確率的（揺れる） | 決定論的（版が同じなら同一） |
| 版を変えたとき | プロンプトを変えれば対応可 | **再インデックス必須**（ベクトルが変わる） |
| インデックスへの影響 | — | 新旧の版を混在させると類似度が壊れる |
