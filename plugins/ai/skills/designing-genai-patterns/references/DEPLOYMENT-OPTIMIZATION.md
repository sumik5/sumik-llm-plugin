...
### コンテキストウィンドウ予算管理（入出力合算・課金）

コンテキストウィンドウ（max tokens）は**入力と出力の合計**を覆う上限である。出力ぶんの余白を残さずに入力を詰めると、回答が途中で切れるか、上限超過でリクエストが拒否される。

- 例: 8192トークン窓で入力が200トークンなら、出力に使える余白は残り7992トークンのみ。入力を増やすほど出力余白は減る。
- 課金は**入力+出力の総トークン数**で発生する。長いウィンドウを使い切るほどコストも増える。
- トークン数の数え方はプロバイダのトークナイザに依存し、同じ文章でもモデルごとに微妙に変動する。送信前にトークナイザでカウントして余白を確認する。

→ 入力サイズは「期待する出力長ぶんの余白を引いた残り」で設計する。余白が足りない場合は、プロンプト圧縮（本パターン）や Prompt Caching（P25）で入力を削る。
