...
### 関連手法: Wait-injection（budget forcing）

モデルが生成を終了しようとするタイミングで終了トークン（EOS）を "Wait" で上書きし、継続生成させて直前の内容を再評価させる手法。最終応答は初期応答より内省的になる傾向がある。

**実装制約（重要）**: EOS 上書きは生成ループのデコード段階への低レベル介入を要する。**OSS 推論ライブラリと自前ホスティング**（モデルの重みとデコードにアクセスできる環境）が前提であり、多くの hosted API では直接実装できない。hosted API 環境では「再考プロンプトで再呼び出し」の近似になり**追加のモデル呼び出しが発生する**。「追加呼び出し不要で内省を得られる」のは自前デコード制御が可能な場合に限る。

**使い分けの指針**:

| 環境 | 推奨 |
|------|------|
| OSS推論ライブラリ + 自前ホスティング | Wait-injection（デコード制御で追加呼び出し不要） |
| hosted API のみ利用可能 | P18 Reflection の反復ループの方が現実的 |
