...
- **React**: 19.2（Actions、useActionState、ref as prop、View Transitions、useEffectEvent、`<Activity/>`）
- **TypeScript**: 5.x（最新安定5.9 / Next.js 16 最低要件5.1+。strict mode + noUncheckedIndexedAccess + exactOptionalPropertyTypes / verbatimModuleSyntax 等の厳格オプション）
...
- **Node.js**: 開発 24.x（Current）/ 本番 22.x（LTS）。Next.js 16 の最低要件は **20.9+（LTS）**（Node 18 はサポート外）
- **pnpm**: 最新版（Corepack経由）
...
- React 19新機能、Internals、パフォーマンスルール → **`developing-react`** スキル参照
...
- **[NP-MIGRATION-FROM-PAGES.md](./references/NP-MIGRATION-FROM-PAGES.md)**: Pages Router → App Router 段階移行（API対応関係・"use client"境界・per-fetch revalidate・両ルーター共存規則）
...
- **`mastering-typescript`**: any/Any型禁止、型ガード、厳格な型チェック
...
### React固有スキル
- **`developing-react`**: React 19新機能、Internals、パフォーマンスルール（react-doctor 47+ルール）、アニメーション、RTLテスト
...
4. **[NP-MIGRATION-FROM-PAGES.md](./references/NP-MIGRATION-FROM-PAGES.md)** - 既存 Pages Router プロジェクトの App Router 段階移行
5. **[DATABASE.md](./references/DATABASE.md)** - Prisma ORMでデータ層を構築
6. **[TESTING.md](./references/TESTING.md)** - テスト戦略を学ぶ

### 上級者向け
1. **[TOOLING.md](./references/TOOLING.md)** - 開発ツールの最適化
2. **[DOCKER.md](./references/DOCKER.md)** - 本番環境デプロイ
3. `writing-clean-code`、`mastering-typescript`、`securing-code` - コード品質向上
...
### 1. 型安全性（必須）
- **any/Any型禁止**: TypeScript/Python共通（`mastering-typescript`スキル参照）
...
> **React固有のコンテンツ**（Internals、Performance rules、アニメーション、RTL）は **`developing-react`** スキルに移動しました。
