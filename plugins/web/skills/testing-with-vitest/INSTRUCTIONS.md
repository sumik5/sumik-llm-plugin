...
Vitest はフレームワーク非依存の Vite ネイティブテストランナーで、v4.x は **Node.js >= 20.0.0** および **Vite >= 6.0.0** を要件とする。汎用テスト方法論（TDD/AAA/モック戦略）は `testing-code` スキルを参照。React コンポーネントテスト（RTL）は `developing-react` を参照。ブラウザ E2E は `testing-e2e-with-playwright` を参照。
...
- **developing-react** — React Testing Library（RTL）・コンポーネントテスト・`renderHook`
