...
description: >-
  Agent Teamオーケストレーション（チーム編成・タチコマ並列起動・進捗管理）。
...
**Claude Code本体は最小限の判断（並列化の要否）のみ行い、計画策定をplanner タチコマに委譲、実装をimplementer タチコマ複数に並列委譲するスキル。Agent Team API（TeamCreate, TaskCreate, Task, SendMessage, TeamDelete）で管理。**
...
Claude Code（このスキルをロード）
    ├─ Phase 1: TeamCreate → planner タチコマ起動 → コードベース分析・docs/plan作成
    ├─ 計画レビュー・承認（ユーザー確認）
    └─ Phase 2: TaskCreate → implementer タチコマ複数を並列起動 ← tmux pane ✓
                 → 進捗管理 → 統合 → クリーンアップ
```
...
### Phase 1: 計画策定（planner タチコマに委譲）
1. **TeamCreate** - チーム作成
2. **planner タチコマ起動（model: opus）** - コードベース分析・要件整理・`docs/plan-{feature}.md` 作成
3. **計画レビュー・承認** - ユーザー確認

### Phase 2: 実装（implementer タチコマ並列起動）
4. **TaskCreate + Task tool（run_in_background: true）** - plan に基づきタチコマ複数を並列起動（model: sonnet）
5. **進捗管理 → 統合 → クリーンアップ** - SendMessage、TaskList、TeamDelete

**Claude Code本体の役割は「並列化判断 → TeamCreate → planner起動 → 計画レビュー → implementer起動 → 進捗監視」に限定。** ファイル読み込みや詳細分析はplanner タチコマが担当する。
...
- **`run_in_background: true` を省略しない**（並列実行が機能しない）
...
- `testing-e2e-with-playwright` - E2Eテスト作成（testerメンバー向け）
