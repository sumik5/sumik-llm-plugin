...
**Claude Code本体がAgent Team API（TeamCreate, TaskCreate, Task, SendMessage, TeamDelete）を直接操作してチーム並列実行を管理するスキル。**
...
### After（このスキル方式）
```
Claude Code（このスキルをロード）→ TeamCreate/TaskCreate/Task tool → タチコマ
                                                              ↓
                                                    tmux制御が効く ✓
```

**Claude Code本体が直接公式Agent Team APIを呼び出すことで、tmux統合と安定性が向上します。**
...
1. **`docs/plan-{feature}.md` 作成** - タスクリスト・ファイル所有権パターン定義
2. **TeamCreate** - チーム作成
3. **TaskCreate** - タスク一覧作成（5-6タスク/メンバー目標）
4. **Task tool（run_in_background: true）** - タチコマ並列起動
