...
Reactアプリケーションのパフォーマンス最適化は、ユーザー体験の質を左右する重要な要素です。このドキュメントでは、レンダリングの仕組みとメモ化戦略を解説します。

---

## 目次

1. [Reactレンダリングの基本原理](#reactレンダリングの基本原理)
2. [メモ化戦略](#メモ化戦略)
3. [依存配列の理解](#依存配列の理解)
...
**重要**: renders、initializers、effects、state settersは純粋関数にすること（外部入力・外部副作用を持たない）。
...
メモ化には3つの主要なAPI：`memo()`、`useMemo`、`useCallback`があります。

| API | 用途 | 使用場面 |
|-----|------|---------|
| `memo()` | コンポーネント全体をメモ化 | 大規模リスト、高コストなレンダリング |
| `useMemo` | 値をメモ化 | 高コスト計算、オブジェクト/配列の安定化 |
| `useCallback` | 関数をメモ化 | メモ化コンポーネントへのコールバック |
...
**注意**: `useCallback`は`useMemo`の簡略版であり、以下は等価です：
...
**重要**: 空配列と配列なしは正反対の意味です。

| 依存配列 | 実行タイミング |
|---------|--------------|
| なし（引数省略） | 毎回のレンダー後 |
| `[]`（空配列） | マウント時のみ（クリーンアップはアンマウント時） |
| `[dep]`（非空） | マウント時 + `dep`変更時 |
...
**React 19の新機能**: React Compiler（実験的）が自動メモ化を提供する予定ですが、現時点では手動メモ化の理解が重要です。
