...
全プロジェクトのセッションログ（`~/.claude/projects/*/*.jsonl`）を確認し、以下の観点でスキル利用状況をレビューする：

1. **どのスキルが呼ばれたか**（Skillツール呼び出しの確認）
2. **スラッシュコマンドの実行状況**（コマンド名別の利用頻度）
3. **未使用スキルの特定**（呼び出しが確認できないスキル一覧）
...
- [ ] セッションログ（`~/.claude/projects/*/*.jsonl`）を手動でレビューしてスキル利用状況を確認
