...
> **「プロセス中の確認」と「コンテンツへの埋め込み」の区別**
>
> - **プロセス中の確認**: スキルを*使うプロセス中*に Claude がユーザーに確認する（スキルの説明に「AskUserQuestion で確認する」と記述する）
> - **コンテンツへの埋め込み**: *作成されるスキル自体*の INSTRUCTIONS.md に判断分岐パターンを組み込む（スキルの利用者が判断分岐できるようにする）
>
> 両者は別の目的であり、混同しないこと。
...
| 環境依存の設定がある（クラウドリージョン・ライセンス等） | 推奨: 確認後に手順を分岐 |
| スキルの主な利用者が自動エージェント | 注意: `background: true` 制約を考慮 |
