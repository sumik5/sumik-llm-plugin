...
hooks:                                  # スキルスコープのライフサイクルフック
  PreToolUse:
    - matcher: Write
      hooks:
        - command: "validate.sh"
license: MIT                           # オープンソース公開時のライセンス（MIT, Apache-2.0等）
compatibility: >-                      # 環境要件（1-500文字）
  Requires macOS or Linux. Network access needed for MCP integration.
metadata:                              # カスタムキーバリューペア
  author: ProjectHub
  version: 1.0.0
...
| `license` | なし | オープンソース公開時のライセンス（MIT, Apache-2.0等） |
| `compatibility` | なし | 環境要件（対象プロダクト、OS、ネットワーク等。1-500文字） |
| `metadata` | なし | カスタムキーバリューペア（author, version, mcp-server等） |
