...
| # | 項目 | 質問文案 | 選択肢 | 回答 → 反映先 |
|---|------|---------|--------|--------------|
| 1 | **テーマ** | スライドの配色テーマはどうしますか？ | ・ライト白背景・青アクセント (推奨)<br>・ダーク背景<br>・ブランド配色に合わせる | ライト → デフォルト（`theme/sample.css` のまま）。ダーク → `theme/dark.css` を作成し各デッキの `<link>` を差し替え（「### ダークテーマを作る場合」参照）。ブランド → `:root` 変数を上書き |
...
| 3 | **フォント** | 本文フォントはどうしますか？ | ・システム標準（埋め込みなし・推奨）<br>・ブランドフォントを使う | システム標準 → 変更なし。ブランド → `.woff2` を `shared/fonts/` へ同梱し `@font-face` 定義 + `--font-sans` を差し替え（CDN直リンク禁止） |
...
| 5 | **情報密度** | 1枚あたりの情報量はどうしますか？ | ・標準 (推奨)<br>・高密度・ハンズオン<br>・余白多め・講演向け | 標準 → デフォルト。高密度 → 第一選択は「スライド分割」、収まらない場合のみ「情報密度の高いデッキ向けCSS調整」節の縮小オーバーライドを適用。講演向け → 余白を広めに、1枚の要素数を絞る |
| 6 | **シンタックスハイライト** | コードに色付け（シンタックスハイライト）しますか？ | ・コードあり → 有効 (推奨)<br>・コードなし → 無効 | 有効 → デフォルト（engine が自動適用）。主要言語を併せて確認し `<code class="language-xx">` を付与。無効 → デッキに `<div class="deck" data-no-highlight>` を付ける（後述「### シンタックスハイライト」参照） |
| 7 | **言語** | スライドの言語は？ | ・日本語 (推奨)<br>・英語<br>・日英併記 | 日本語 → デフォルト。英語 → ENGLISH-* 系リファレンス（表現・発音）を参照。日英併記 → 主要見出しに英語併記 |
| 8 | **ロゴ** | ロゴは何を使いますか？（**必ず確認する。勝手にロゴを決めない**） | ・指定ロゴを使う（パス or 画像を提供してもらう）<br>・プレースホルダー `shared/logo/logo.svg` のまま (推奨/未定時)<br>・ロゴなし | 指定 → `shared/logo/` に配置し各スライドに `<img class="slide-logo" src="../../shared/logo/{file}" alt="Logo" onerror="this.style.display='none'">`。プレースホルダー → 既定の `logo.svg`（差し替え前提）。なし → `<img class="slide-logo">` を置かない。配置面はタイトル・コンテンツ・エンディングのみ（ランディング・セクション区切りには置かない） |
...
| 10 | **素材忠実度・QCD** | 素材の扱いと優先軸は？（2軸） | ・原文を極力保持 (推奨) / 発表用に要約・再構成可<br>・品質優先 / 納期優先 | 原文保持 → 「ソース素材の変換ルール」を厳守。要約可 → 1メッセージ化を優先。品質優先 → リハーサル・はみ出し検証を厚く。納期優先 → 最小構成で素早く |
...
タチコマ（スライド）がスライド作成前にAskUserQuestionで収集すべき情報。回答に応じてスライド構成・スタイル・メッセージ設計を最適化する。
...
プレゼンの品質向上に関する詳細知識は `references/` 配下のファイルを参照する。タチコマ（スライド）はこれらの知識をプリロードしていないため、必要に応じて各ファイルを読み込んで活用する。
