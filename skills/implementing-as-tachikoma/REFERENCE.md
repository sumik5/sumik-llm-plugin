...
Developer Agentは、Managerへの報告を以下のフォーマットで行います。
...
状況：40%完了（認証部分実装済み）
完了予定：ブロッカー解消後、2時間
課題：API仕様書の一部が不明確。Managerに確認をお願いします。
...
Developer Agentは、Managerからの指示がない限り、以下の行動を絶対に行いません。
...
- 指示系統を明確にするため（PO → Manager → Developer）
- 不要なコンテキスト消費を避けるため
- Managerの計画を尊重するため
...
※ Git操作は実施していません。
  変更内容の確認とコミットはユーザー/Managerにお願いします。
...
- Managerや他のDeveloperが作業内容を把握する必要がある
...
- **coordinating-as-kusanagi** - Managerとのコミュニケーション
