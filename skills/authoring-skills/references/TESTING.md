...
スキルのテストには3つのレベルがある。品質要件とスキルの可視性に応じて選択する:

| テストレベル | 説明 | 適用場面 |
|------------|------|---------|
| **Manual testing（Claude.ai）** | 直接クエリを実行し挙動を観察。セットアップ不要、高速イテレーション | 個人・小規模チーム向け |
| **Scripted testing（Claude Code）** | テストケースを自動化し、変更時の繰り返し検証 | チーム共有スキル |
| **Programmatic testing（Skills API）** | 評価スイートを構築し、定義済みテストセットに対して体系的に実行 | エンタープライズ・大規模展開 |

### Skills API による配布とテスト

プログラマティックなユースケース（アプリケーション構築、エージェント、自動ワークフロー）では、APIがスキルの管理と実行を直接制御する。

**主要機能:**
- `/v1/skills` エンドポイント: スキルの一覧取得・管理
- `container.skills` パラメータ: Messages APIリクエストにスキルを追加
- Claude Console経由のバージョン管理
- Claude Agent SDKとの連携でカスタムエージェント構築

**APIとClaude.ai/Claude Codeの使い分け:**

| ユースケース | 推奨サーフェス |
|------------|---------------|
| エンドユーザーがスキルを直接利用 | Claude.ai / Claude Code |
| 手動テスト・イテレーション | Claude.ai / Claude Code |
| アドホックなワークフロー | Claude.ai / Claude Code |
| アプリケーションからプログラマティックに利用 | API |
| 大規模プロダクションデプロイ | API |
| 自動パイプライン・エージェントシステム | API |

**Agent Skills はオープンスタンダード:** MCPと同様、スキルはプラットフォーム間で移植可能であるべきという思想。`compatibility` フィールドで特定プラットフォーム依存を明記可能。
