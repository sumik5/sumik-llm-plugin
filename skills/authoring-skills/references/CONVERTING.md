...
| EPUB | `.epub` | `pandoc` CLIコマンドでMarkdown変換（画像ベースの場合は `recognize-image.py` で追加処理が必要） |
...
pandocでEPUBをMarkdown変換した結果にテキストがほとんどなく `![` による画像参照が大半を占める場合は**画像ベースEPUB**と判定し、以下の追加処理を実施する。

1. **EPUBから画像を抽出**（EPUBはZIPファイル）:
   ```bash
   mkdir -p /tmp/epub-images
...
4. **AskUserQuestionでOCR変換の可否を確認**（recognize-image.py 実行前に必須）:
   ...
   AskUserQuestion(
       questions=[{
           "question": "画像ベースのEPUBが検出されました。LM Studio OCRで画像をテキストに変換しますか？\n\n- 対象: {epub_filename}\n- 画像枚数: {image_count}枚\n- ※ LM Studioが起動しモデルがロードされている必要があります",
           "header": "画像OCR変換の確認",
           "options": [
               {"label": "変換する", "description": "recognize-image.py で全画像をOCR変換"},
               {"label": "スキップ", "description": "画像変換せず、テキスト部分のみでスキル作成を進める"}
           ],
           "multiSelect": False
       }]
   )
...
| EPUB | チャプター数 × 1000 | 下回ったら警告（テキスト不足は画像ベースの可能性あり → 「画像ベースEPUBの検出と変換」参照） |
| EPUB（recognize-image.py変換後） | - | 目視確認（日本語テキストが正常に抽出されているか、ページ順序が正しいか確認） |
