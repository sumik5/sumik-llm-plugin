...
## 7. Dynamic Context Injection（動的コンテキスト注入）

### 使用場面

スキルの本文にリアルタイム情報や外部コマンドの実行結果を動的に注入したい場合。

### 構造

SKILL.md の本文内で `` !`command` `` 構文を使用すると、スキルロード時にコマンドが実行され、その出力がコンテキストに注入される。
...
- **動的情報の注入**: スキルロード時にコマンドが実行され、最新の状態を取得
- **プロジェクト固有のコンテキスト**: ブランチ、最近の変更、ファイル構成を自動取得
- **静的ガイドラインとの組み合わせ**: 動的情報と静的なベストプラクティスを組み合わせて使用
- **注意**: コマンド実行によるセキュリティリスクを考慮し、信頼できるコマンドのみ使用する

---

## 8. Subagent Execution（サブエージェント実行）

### 使用場面

メインコンテキストを汚染せずに、独立した調査・分析タスクを実行したい場合。

### 構造

フロントマターで `context: fork` を指定すると、スキルはサブエージェントとして別コンテキストで実行される。`agent` フィールドでエージェントタイプを指定可能。
...
description: >-
  Investigates codebase structure and reports findings.
  Use when exploring unfamiliar code or analyzing dependencies.
context: fork
agent: Explore
allowed-tools: Read, Grep, Glob
disable-model-invocation: true
argument-hint: "[調査対象のディレクトリ or パターン]"
---
...
- **コンテキスト分離**: `context: fork` でメインの会話コンテキストを消費しない
- **エージェントタイプの選択**: `Explore`（読み取り専用）、`Plan`（設計向け）など用途に応じて選択
- **ツール制限**: `allowed-tools` でサブエージェントが使用可能なツールを限定
- **引数の受け渡し**: `$ARGUMENTS` で呼び出し時の引数をスキル本文に展開

---

## 9. Visual Output Generation（ビジュアル出力生成）

### 使用場面

スキルの出力としてリッチなビジュアル（HTML、SVG、ダイアグラム等）を生成する場合。
...
生成したHTMLファイルをローカルに保存し、ブラウザで開ける形式にする。
...
<html>
<head>
  <title>$ARGUMENTS Dashboard</title>
  <style>
    /* スタイルガイドに基づくCSS */
  </style>
</head>
...
1. データを収集・分析
2. HTMLテンプレートにデータを注入
3. ファイルとして保存（`/tmp/dashboard-${CLAUDE_SESSION_ID}.html`）
4. ユーザーにファイルパスを報告
````
...
## 10. Extended Thinking（拡張思考）

### 使用場面

複雑な分析や多段階の推論が必要なタスクで、Claudeの思考プロセスをより深くする場合。
...
- **`ultrathink` キーワード**: スキル本文に含めることで、Claudeの拡張思考モードを有効化
- **配置場所**: スキル本文の冒頭近くに配置するのが効果的
- **使用場面**: 複雑なアーキテクチャ分析、多ファイル横断のリファクタリング計画、セキュリティ監査等
- **注意**: すべてのスキルに付けるのではなく、本当に深い思考が必要なスキルにのみ使用する
