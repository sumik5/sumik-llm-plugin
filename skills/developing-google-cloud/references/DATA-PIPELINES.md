...
DataformはBigQuery上のELTワークフローを、SQL拡張ファイル（SQLX）で宣言的に管理するサービス。複数SQLの依存関係管理・バージョン管理・データ品質テスト・スケジュール実行を統合する。

### SQLXファイル構造

SQLXは **configブロック** と **bodyブロック** の2層で構成される。

```sql
config {
  type: "table",            -- 出力形式: table / view / operations / assertion
  description: "日別DAU集計",
  columns: {
    dt: "集計対象日",
    paid_users: "課金ユーザー数"
  ...
  assertions: {
    uniqueKey: ["dt"],
    nonNull: ["dt", "paid_users"]  -- 組み込みアサーション（品質テスト）
  }
}
SELECT
...
FROM
  ${ref("work_events")} e  -- 依存テーブルを宣言（DAGを自動生成）
INNER JOIN users u ON u.user_pseudo_id = e.user_pseudo_id
```
...
| type | 用途 |
|------|------|
| `table` | SELECT結果をテーブルとして永続化 |
| `view` | SELECT結果をビューとして定義 |
| `incremental` | 増分データのみ追記（大規模テーブル更新コスト削減） |
| `operations` | LOAD DATA等の結果を返さないSQL（`hasOutput: true`で他ファイルから参照可能） |
| `assertion` | 手動データ品質テスト（SELECTが0行 = テスト合格） |

### 依存関係とDAG

`${ref("table_name")}` により依存テーブルを宣言すると、Dataformが実行順序を自動解決する。COMPILED GRAPHビューでDAGを視覚的に確認できる。

### スケジュール実行

リリース構成（SQLXのコンパイル頻度）とワークフロー構成（SQL実行頻度）を分けて設定する。ビルドが実行中にワークフローが起動しないよう、**ビルド間隔は実行間隔より短く**設定する。cron形式で指定（例: `0 9,12 * * *`）。

### 本番運用の推奨事項

- **変数の外部化**: `workflow_settings.yaml` の `vars` セクションで環境ごとのパラメータを管理
- **アサーション配置**: データ品質テストをDAGの依存関係に組み込み、品質不合格時は後続処理を自動停止
- **増分テーブル**: データ量増加に備え `type: "incremental"` で追記型設計を採用
- **JavaScript共通化**: 複数SQLXで共用するカラム定義やロジックは `includes/` 配下のJSファイルに集約
- **オーケストレーター選択**: 既存ワークフローに合わせてCloud Composer / Workflows からも呼び出し可能

> **クロスリファレンス**: Dataflow SQL（ストリーミングSQL）については [REALTIME-ANALYTICS.md](./REALTIME-ANALYTICS.md) を参照。
