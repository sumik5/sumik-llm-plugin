...
description: >-
  ...
  For Docker-specific patterns, use managing-docker instead. For general monitoring design, use designing-monitoring instead. For code-level security (OWASP, CodeGuard), use securing-code instead. For data architecture patterns (CQRS, event sourcing), use architecting-data instead. For AWS development (Lambda, CDK, EKS, Bedrock), use developing-aws instead. For serverless security (GCS hacking, Cloud Run abuse, privilege escalation), use securing-serverless instead.
---
