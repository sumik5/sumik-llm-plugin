...
## TypeScriptコンパイラの振る舞いとモダン設定

TypeScriptコンパイラの実践的な振る舞い、モダンな設定、落とし穴回避の実践ガイド。

| ファイル | 内容 |
|---------|------|
| [TT-COMPILER-BEHAVIOR.md](./references/TT-COMPILER-BEHAVIOR.md) | satisfies、余剰プロパティ（Fresh/Stale）、Open/Closed Objects、Evolving any、Function Assignability、Deriving vs Decoupling |
| [TT-MODERN-CONFIG.md](./references/TT-MODERN-CONFIG.md) | 推奨tsconfig決定木、Enums/Namespaces注意点、erasableSyntaxOnly、Generic Function vs Type |
| [TT-PRACTICAL-WORKFLOW.md](./references/TT-PRACTICAL-WORKFLOW.md) | IDE駆動ワークフロー（hover/補完/quick fix/リファクタリング）、The Weird Parts（Evolving any/Fresh vs Stale/{}型/Type World vs Value World 等の落とし穴） |

---

## TypeScriptクリーンコード実践

TypeScriptプロジェクトのスケーリング、パフォーマンス最適化、契約進化パターン、エラーハンドリングの実践ガイド。

| ファイル | 内容 |
|---------|------|
| [CC-PERFORMANCE.md](./references/CC-PERFORMANCE.md) | プロファイリングツール選択、ボトルネック検出パターン、最適化テクニック（Lazy Loading/Tree Shaking/キャッシュ）、パフォーマンス vs 可読性トレードオフ |
| [CC-PROJECT-SCALING.md](./references/CC-PROJECT-SCALING.md) | Feature/Function/Hybrid構成比較、ESM/CJS選択、npm/pnpm/yarn比較、Monorepo vs Polyrepo判断基準、Nx workspace、ADR、Husky/lint-staged |
| [CC-CONTRACT-EVOLUTION.md](./references/CC-CONTRACT-EVOLUTION.md) | Contract Drift検出チェックリスト、Runtime Validation Gate（Zod連携）、Vendor Abstraction（Anti-Corruption Layer）、Discriminated Unions、OpenAPI→Orval契約駆動開発 |
| [CC-ERROR-PATTERNS.md](./references/CC-ERROR-PATTERNS.md) | TypeScript固有エラー型分類、catchブロックのunknown型、非同期エラーハンドリングパターン、デバッグツール活用（source maps/VS Code設定） |
| [CC-DOCUMENTATION.md](./references/CC-DOCUMENTATION.md) | TSDoc拡張タグ(@remarks/@link)、TypeDocセットアップ(typedoc.json/npm script)、静的ドキュメントサイト生成、TSDoc/TypeDoc採用判断 |

---

## TypeScript実践ガイド（デコレータ・型関係）

TypeScript 5.0対応のデコレータ完全ガイドと、集合論に基づく型関係の理解を提供。

| ファイル | 内容 |
|---------|------|
| [GG-DECORATORS.md](./references/GG-DECORATORS.md) | TS 5.0デコレータ（メソッド/ゲッター/セッター/フィールド/クラス/Auto-Accessor）、デコレータファクトリ、型付きデコレータ |
| [GG-TYPE-RELATIONS.md](./references/GG-TYPE-RELATIONS.md) | 型を集合として捉える型関係論、サブタイプ/スーパータイプ、構造的部分型、関数型互換性（反変性）、satisfies |

---

## 型推論と型安全の実践パターン

型推論の振る舞いパターンと型安全の実践テクニックを提供。

| ファイル | 内容 |
|---------|------|
| [JT-TYPE-INFERENCE-PATTERNS.md](./references/JT-TYPE-INFERENCE-PATTERNS.md) | Widening Literal Types、Promise型推論、JSON型推論、import推論 |
| [JT-TYPE-SAFETY-PATTERNS.md](./references/JT-TYPE-SAFETY-PATTERNS.md) | Weak Type、Excess Property Checks、upcast/downcast、宣言空間、モジュール型拡張 |
