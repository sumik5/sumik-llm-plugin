...
**Codex本体はオーケストレーターに徹し、tachikoma-architecture agent に計画策定を委譲する。計画承認後、ドメイン別専門agentを自然言語で起動し、`config.toml` の `max_threads` による並列制御に委ねる。**

---

## ⚡ Claude Code との違い

| 項目 | Claude Code（orchestrating-teams） | Codex（このスキル） |
|------|----------------------------------|-------------------|
| Agent起動方式 | `TeamCreate` + `Agent tool call` API | 自然言語で指示（例: "Spawn one agent per point"） |
| 並列制御 | `run_in_background: true` + SendMessage | `config.toml` の `max_threads`（デフォルト6） |
| スレッド管理 | tmux pane | `/agent` コマンド |
| Team API | 利用可 | ❌ 使用不可 |

**Codex環境ではClaude Code Team API（TeamCreate/SendMessage等）は使えない。**
