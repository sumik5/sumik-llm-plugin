...
- **practicing-devops** — Docker/コンテナパターン、DevOps方法論・CI/CDパイプライン設計
- **implementing-observability** — クラウド非依存の監視・オブザーバビリティ設計
- **designing-genai-patterns** — クラウド非依存のRAGシステム構築・LLMアプリケーション設計
- **designing-genai-patterns** — LLMアプリケーション運用フレームワーク
