...
## 🏗️ オブジェクト指向UIデザイン（OOUI）

UIの構造設計にオブジェクト指向アプローチを適用する手法。タスク指向（動詞→名詞）ではなく、オブジェクト指向（名詞→動詞）でUIを設計する。
...
| 特徴 | タスク指向UI | オブジェクト指向UI |
|------|------------|------------------|
| 操作順序 | アクション→対象 | 対象→アクション |
| ナビゲーション | 機能メニュー | オブジェクト一覧 |
| 画面構成 | 機能別画面 | オブジェクト別画面（コレクション/シングル） |
| ユーザーメンタルモデル | 手順を覚える | 対象を見つける |
| 例 | 「新規作成」→入力フォーム | 一覧→選択→編集 |

### 設計3ステップ（概要）

1. **オブジェクトの抽出**: 要件から名詞を抽出→汎化→メインオブジェクト特定→プロパティ・アクション定義
2. **ビューとナビゲーション**: コレクション/シングルビューの設計、ルートナビゲーション構造
3. **レイアウトパターン適用**: 配置パターン・表示形式・CRUDアクションパターンの選択

### 詳細リファレンス

- **[OOUI-METHODOLOGY.md](./references/OOUI-METHODOLOGY.md)**: 3ステップ設計プロセスの詳細手順
- **[OOUI-PATTERNS.md](./references/OOUI-PATTERNS.md)**: レイアウト・ビュー・アクションパターンカタログ
