...
2. **プラットフォーム用語置換**: Body 全文と description 中の `Claude Code` を `Codex` に一律置換する
...
- sandbox_mode: permissionMode が plan または tools に Write/Edit がなければ "read-only"、それ以外 "workspace-write"
...
7. 生成した .toml に「Claude Code」が残っていないか最終確認する
...
3. **プラットフォーム用語置換**: `Claude Code` → `Codex` 一律置換
...
- **プラットフォーム用語の置換忘れ防止**: 生成した `.toml` ファイルに `Claude Code` が残っていないか最終確認する。残っている場合は置換漏れとして修正する
