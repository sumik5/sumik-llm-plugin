...
- 各フィールドのデフォルト値
- 新機能・変更点
- **INSTRUCTIONS.md との比較用に以下も記録する**:
  - 推奨モデル名（最新のデフォルトモデル）
  - 利用可能な sandbox_mode の選択肢一覧
  - 新規追加・廃止されたフィールド

#### Step 1.1: スキル自己改善チェック（公式仕様との同期）

Step 1 で取得した最新仕様を、**このスキル自体（INSTRUCTIONS.md）の記述内容**と比較し、差分を検出する。

**比較対象**:

| チェック項目 | INSTRUCTIONS.md 内の該当箇所 | 差分の例 |
|-------------|---------------------------|---------|
| 利用可能フィールド | フィールドマッピングテーブル（Step 3） | 新フィールドが追加されている |
| デフォルト値 | フィールドマッピングの変換ルール列 | sandbox_mode のデフォルトが変更 |
| 推奨モデル名 | テンプレート内の `model` 値 | 新モデルがリリースされている |
| sandbox_mode の選択肢 | sandbox_mode 判定ルール | 新しい sandbox タイプが追加 |
| 新機能・廃止機能 | ワークフロー全体 | 新しい設定項目、廃止された機能 |

**実行フロー**:

1. Step 1 で取得した仕様サマリーと、INSTRUCTIONS.md の以下セクションを比較する:
   - フィールドマッピングテーブル
   - Agent .toml テンプレート
   - Worker プロンプトテンプレート内のモデル名・フィールド
   - 変換例
2. 差分が**ない**場合: 「スキル記述は最新仕様と一致しています」と報告し、Step 1.5 へ進む
3. 差分が**ある**場合:
   - 検出した差分を一覧で表示する
   - AskUserQuestion で以下を確認する:
     - 「INSTRUCTIONS.md を更新しますか？」（はい / いいえ / 変換後に更新）
   - 「はい」の場合: INSTRUCTIONS.md の該当箇所を Edit ツールで更新し、Step 1.5 へ進む
   - 「いいえ」の場合: 更新せず Step 1.5 へ進む
   - 「変換後に更新」の場合: 変換処理を先に完了し、最後に INSTRUCTIONS.md を更新する

> **注意**: このステップはスキルの品質維持のために毎回実行する。公式仕様の変更を早期に検出し、変換精度の劣化を防ぐ。
...
> **※ モデル名は Step 1 で取得した最新値を使用すること**（以下の例は執筆時点の値）
