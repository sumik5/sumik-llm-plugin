...
> ⚠️ **タグはデッキ横断グローバル — `tag:` 単独検索の誤爆**: Anki のタグはコレクション全体で共有されるため、`find_notes({ query: "tag:_要手修正" })` のようなタグ単独検索は**別デッキ・別プロジェクトのカードまで巻き込む**（実観測: あるデッキの6枚を狙ったつもりが、同名タグを使う無関係デッキを含め420件ヒットした）。特定デッキ内のタグ付きカードだけを対象にするには、必ず `deck:"デッキ名" tag:タグ名` と **deck と tag を AND** でスコープすること。件数監査（桁レベルの集計）も同様にデッキスコープを付けないと誤った母数で判断してしまう。
