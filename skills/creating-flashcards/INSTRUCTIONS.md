...
- **日本語**: そのまま処理
- **英語（またはその他の外国語）**: `converting-content` スキルのワークフローを使用してLM Studioで日本語に翻訳し、カードを作成する。原文は折りたたみセクションとして保持する（詳細は後述の「多言語対応」セクション参照）。セッション内初回の翻訳時はモデル選択が必要。
...
> ⚠️ **シリーズ書籍のデッキ階層**: 同一資格・同一級でも複数の問題集シリーズを扱う可能性がある場合、`{資格名}::{級}::{シリーズ識別子}` のように3階層以上のサブデッキ命名を推奨する。級レベルにフラットに保存すると、別シリーズが増えたとき再編成（多数カードの `changeDeck`）が必要になる。最初から1階層深く切るとAnkiの階層タグ機能と併用してフィルタリングしやすい。シリーズ識別子には書籍タイトルの汎用識別子や刊行年など、内容を一意に識別できる文字列を使用する。

> ⚠️ **年度版投入時のユーザー意図確認 — 差分投入 vs 独立リソース投入**: 同一資格試験対策書籍の新年度版を投入する際、「前年版との差分のみ追加したい」と「新年度版を独立した学習リソースとして丸ごと投入したい」では方針が真逆になる。差分判定で既存扱いされた件数が大幅に多い場合（例: 新規扱いが全体の10%以下）、ユーザーが独立リソース投入を期待しているにもかかわらず差分モードで動いている可能性が高い。**判断フロー**: (1) 年度版の投入セッション開始時に AskUserQuestion で「差分追加か独立投入か」を確認する、(2) 独立投入を選択した場合は差分判定をスキップして全件投入し、専用サブデッキ（例: `資格名::2026年版`）に配置する。

> ⚠️ **複数版書籍の併存サフィックス命名戦略**: 同じ試験回（例: 第41-44回）を **複数の出版社・複数の版**から取り込む場合、既存デッキを上書きすると元の出版社版が失われる。**対策**: 既存デッキと並存させる場合は `過去問題第N回学科(YYYY-YYYY版)` のように **版年度サフィックス**を付けて別デッキ化する。サフィックス例: `(2023-2024版)`, `(出版社略称)`, `(電子書籍版)`。同名サブデッキは Anki UI で並列表示されるため、ユーザーは両者を見比べながら学習できる。**判断フロー**: `findNotes deck:"<予定デッキ名>"` で件数を確認し、(a) 0件 → そのままの命名、(b) ある程度埋まっている → サフィックス付与で併存、(c) 1件だけのプレースホルダー → ユーザーに「上書き/併存」を AskUserQuestion で確認。フォーマット差（別出版社の電子書籍由来で TOC ノイズ含む等）が大きい場合は併存が安全。

> ⚠️ **既存デッキの問番号取得による欠落差分判定**: 既存デッキに「30問あるべきところが28問」のような欠落がある場合、新規パース結果と差分を取れば不足分だけ追加投入できる。**手順**: (1) `findNotes deck:"<既存デッキ>"` で既存ノートIDを取得、(2) `notesInfo` で Question/Answer フィールドから問番号を抽出（regex例: `[フッター内の]問(\d+)` または `<b>問(\d+)\.`）、(3) 期待番号 set（1-30 等）との差で欠落番号を確定、(4) 新規パース結果から欠落番号のカードのみフィルタして投入。**注意**: 既存デッキが別出版社・別フォーマットの場合、問番号埋め込み位置が異なり regex 抽出に失敗することが多い（例: TOCカード1枚 + 本体27枚で28件、内訳が不規則）。その場合は問番号比較ではなく **Front テキスト類似度比較**（`difflib.SequenceMatcher` 0.75以上で同一視）にフォールバックする。実観測: 28件のデッキで問番号 regex が `{1, 3, 29}` の3件しか取れなかったケースあり。問番号抽出失敗時は併存（サフィックス）戦略に切り替えるのが安全。
