...
| 指標 | ファイル1 | ファイル2 | ファイル3 | ファイル4 |
|---|---|---|---|---|
| 追跡期間 | 5年2ヶ月 | 8年3ヶ月 | 13年3ヶ月 | 13年4ヶ月 |
| 元の行数 | 423 | 192 | 227 | 309 |
| 変更されていない行数 | 271 | 101 | 4 | 8 |
| 現在の行数 | 664 | 948 | 388 | 414 |
| 変更回数 | 47 | 99 | 194 | 459 |
| 変更の合計 | 675 | 1,709 | 3,047 | 11,107 |
| 変更率 | 1.6x | 8.9x | 13x | 36x |

**このデータから読み取れる追加の知見**:
- 変更の合計は常に現在の行数より多く、古いファイルほど行の変更が行の追加を上回ります
- ファイル3は13年で161行しか増えていないが、変更合計は3,047行に達しています
- 変更されていない行のほとんどはコメント・ドキュメンテーション・空行です
...
ソフトウェアの基本的な目的はかなり簡潔なものであるべきで、そうすることでシステム全体を現実的に可能なかぎり簡潔にできます。

**目的違反のパターン**:

| パターン | 例 | 結果 |
|---|---|---|
| 無関係な機能の追加 | ワードプロセッサにメール機能 | UIの肥大化、目的の喪失 |
| 作り手とユーザーの目的の不一致 | メール閲覧ソフトの主目的が広告表示 | ユーザーの不満 |
| マーケティング主導の目的拡大 | 「エッジなデザイン」「最先端技術」 | 基本機能の劣化 |

**ユーザー目的との整合**:
- ユーザーの目的達成を困難にすればユーザーを怒らせます
- 成功している製品は単一目的を徹底しています（石鹸、塩、電球）
- ソフトウェアデザイナーは基本目的からの逸脱を防ぐ責任があります
- 効果的なマーケティングに複雑な製品は不要です
...
書籍（付録B）で定義された全原則の分類です。

| 分類 | 内容 |
|---|---|
| **法則** | ソフトウェアの目的、デザインの方程式、変更の法則、不具合発生率の法則、簡潔性の法則、検証の法則 |
| **事実** | 良い/悪いプログラマーの違いは理解力、ソフトウェアを書く人は誰でもデザイナー、簡潔性は相対的、3つの間違い、未来予言の破滅性 |
| **ルール** | 簡潔にコードを書く、デザインは個人で決断、YAGNI拡大版、現在の事実に基づくデザイン、必要な分だけの一般性、一貫性、名前の適切な長さ、コメントは「なぜ」、書き直しの5条件、ラッパー化、インクリメンタルデザイン |
