...
悪いプログラマーは自分が何をしているかを理解せず、良いプログラマーは理解している。ソフトウェアを書くことは純粋な精神的活動であり、理解しているかどうかですべてが決まる。

> **ソフトウェアを書く人は、誰でもデザイナーだ。**

開発チームの一人ひとりが、担当するコードがきちんと設計されていることに責任を持たなければならない。ただし、デザインは民主主義で決まるわけではない。決断は個人で行うべきだ。
...
**デメリットとのバランス**: 変更は助けだけでなくデメリットをもたらすこともある。例えば広告は開発者には役立つがユーザーをいらつかせる。起こりうるデメリットを予測し、メリットでバランスを取ることで変更の価値を計算する。

**ユーザーがいるという価値**: ユーザーのつかない機能にはすぐさま反映されるような価値はない。時間がかかりすぎる変更は価値がゼロになることもある（リリースのタイミングを逸するため）。変更の欲求度を考慮する際にはリリーススケジュールも検討すべき。
...
Em（管理コスト）と Vf（将来の価値）の推移パターンで、実装すべき変更かどうかを判断できる。

**望まれない状態（実装すべきでない変更）**:

| パターン | 日1 | 日2 | 日3 | 日4 | 合計 |
|---------|-----|-----|-----|-----|------|
| Em が Vf より早く増大 | Em=$10/Vf=$1000 | Em=$100/Vf=$100 | Em=$1000/Vf=$10 | Em=$10000/Vf=$1 | Em=$111,110 vs Vf=$1,111 |
| Em・Vf とも増大だが Em が Vf を追い越す | Em=$1000/Vf=$1000 | Em=$2000/Vf=$2000 | Em=$4000/Vf=$3000 | Em=$8000/Vf=$4000 | Em が Vf を超過 |

**望まれる状態（理想的な変更）**:

| パターン | 特徴 | 例 |
|---------|------|-----|
| Em がゼロに収束（最も望ましい） | 管理コストがゼロになれば将来価値の大小を問わず安全 | 日1: Em=$1000/Vf=$0 → 日5: Em=$0/Vf=$10000 |
| Em 減少・Vf 安定（安全） | 安定した価値を低コストで維持 | 日1: Em=$20/Vf=$10 → 日5: Em=$0/Vf=$10 |
| Vf が Em を上回り続ける（許容可能） | 最適ではないが継続して価値が勝る | 日1: Em=$1/Vf=$0 → 日5: Em=$5/Vf=$8 |

**重要な教訓**: 管理にかかる作業量が減少するシステムをデザインするためには、実装にかかる作業量が大きくなることが多い。しかし実装コストはデザイン判断においてほぼ常に無視できる要素である。
