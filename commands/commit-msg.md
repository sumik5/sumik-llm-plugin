...
- gitリポジトリ（`.git` ディレクトリが存在する）内で実行する必要があります
...
1. **差分取得**: `git diff --cached` でステージング済みの変更を取得
2. **スキルロード**: `writing-conventional-commits` スキルをロード
3. **メッセージ生成**: 会話履歴（Claudeのコンテキスト）と差分情報を分析してメッセージを生成
4. **メッセージ表示**: 生成したコミットメッセージをコードブロックで表示
5. **ユーザー確認**: コミット実行・編集・キャンセルの選択を促す

## 手順

### ステップ1: 差分取得

以下のbashコマンドを実行してステージング済みの変更を取得してください：
...
**注意**:
- `git diff --cached` はステージング済みの変更のみを出力します
